...
/********************************************************/
/*   [ScorePdfForm]
/*   「楽譜のPDFデータ」に関するフォーム入力値の
/*    入力値のチェックを行う。
/********************************************************/

@Data
public class ScorePdfForm implements Serializable, BlobValidation{
  ...
  @Length(min=0, max=90)
  @ForeignKey(table="Musical_Score")
  ...
  @NotMovie
  @DataSizeLimit
  private MultipartFile pdf_data_1;
  ...
  @NotMovie
  @DataSizeLimit
  private MultipartFile pdf_data_2;

  @NotImage
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile pdf_data_3;

  @NotImage
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile pdf_data_4;

  @NotImage
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile pdf_data_5;
  ...
  public Boolean blobChecker(Map<String, byte[]> inputblob) throws InterruptedException, ExecutionException {
    ...
    try{
      ...
      throw new InterruptedException("Error location [ScorePdfForm:blobChecker(InterruptedException)]");
    }catch(ExecutionException e){
      throw new ExecutionException("Error location [ScorePdfForm:blobChecker(ExecutionException)]", e.getCause());
    }
  ...
  public CompletableFuture<Boolean> parallelCheck(String filename, byte[] data) {
    ...
    try{
      ...
      if(mime.equals(DatatypeEnum.PDF.getType())){
        result.add(true);
      }else{
        result.add(false);
      }
      ...
      if(ext.equals(DatatypeEnum.PDF.getExt())){
        result.add(true);
      }else{
        result.add(false);
      }
      ...
      throw new NullPointerException("Error location [ScorePdfForm:blobChecker(NullPointerException)]");
    }
