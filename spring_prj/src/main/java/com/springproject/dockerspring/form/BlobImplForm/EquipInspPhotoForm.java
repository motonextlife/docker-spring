...
/********************************************************/
/*   [EquipInspPhotoForm]
/*   「点検情報に関する写真データ」に関するフォーム入力値の
/*    入力値のチェックを行う。
/********************************************************/

@Data
public class EquipInspPhotoForm implements Serializable, BlobValidation{
  ...
  /* テーブル「Equip_Inspect」内に存在する採用考課情報番号のみ設定可能。
  /*******************************************************************/
  @NotBlank
  @Pattern(regexp="^[a-zA-Z0-9]*$")
  @Length(min=0, max=90)
  @ForeignKey(table="Equip_Inspect")
  ...
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_1;
  ...
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_2;

  @NotPdf
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_3;

  @NotPdf
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_4;

  @NotPdf
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_5;

  @NotPdf
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_6;

  @NotPdf
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_7;

  @NotPdf
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_8;

  @NotPdf
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_9;

  @NotPdf
  @NotAudio
  @NotMovie
  @DataSizeLimit
  private MultipartFile photo_data_10;
  ...
  public Boolean blobChecker(Map<String, byte[]> inputblob) throws InterruptedException, ExecutionException {
    ...
    try{
      ...
      throw new InterruptedException("Error location [EquipInspPhotoForm:blobChecker(InterruptedException)]");
    }catch(ExecutionException e){
      throw new ExecutionException("Error location [EquipInspPhotoForm:blobChecker(ExecutionException)]", e.getCause());
    }
  ...
  public CompletableFuture<Boolean> parallelCheck(String filename, byte[] data) {
    ...
    try{
      ...
      if(mime.equals(DatatypeEnum.PHOTO.getType())){
        result.add(true);
      }else{
        result.add(false);
      }
      ...
      if(ext.equals(DatatypeEnum.PHOTO.getExt())){
        result.add(true);
      }else{
        result.add(false);
      }
      ...
      throw new NullPointerException("Error location [EquipInspPhotoForm:blobChecker(NullPointerException)]");
    }
