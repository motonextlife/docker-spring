/** 
 **************************************************************************************
 * @file Facility_Photo_ZipImpl.java
 * @brief 主に[設備写真データ管理]機能で使用する、バイナリデータの圧縮ファイルの作成に関する機能を
 * 実装するクラスを格納したファイル。
 **************************************************************************************
 */ 
...
@Component
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class Facility_Photo_ZipImpl implements Facility_Photo_Zip{
   ...
   * 
   * @details
   * - 抽出の際に用いる、ファイル制限数とデータ制限値に関しては、[画像ファイル]の基準を用いて、その値を
   * 共通クラスに渡す。
   *
  ...
  public List<FileEntity> extractZip(MultipartFile zipfile) throws IOException, ZipException, InputFileDifferException {
    ...
    int count = this.config.getPhoto_list_size();
    int size = this.config.getPhoto_limit();

    try{
      return this.common_zip.extractZipCom(zipfile, count, size);
    }catch(ZipException e){
      throw new IOException("Error location [Facility_Photo_ZipImpl:extractZip(ZipException)]" + "\n" + e);
    }catch(IOException e){
      throw new ZipException("Error location [Facility_Photo_ZipImpl:extractZip(IOException)]" + "\n" + e);
    }
  }
  ...
  public File outputZip(Supplier<List<FileEntity>> samba_generate) throws ZipException, IOException, InputFileDifferException{
    ...
    try{
      ...
      throw new ZipException("Error location [Facility_Photo_ZipImpl:outputZip(ZipException)]" + "\n" + e);
    }catch(IOException e){
      throw new IOException("Error location [Facility_Photo_ZipImpl:outputZip(IOException)]" + "\n" + e);
    }
  }
}
