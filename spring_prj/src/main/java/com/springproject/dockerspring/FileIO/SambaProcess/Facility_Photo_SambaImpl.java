/** 
 **************************************************************************************
 * @file Facility_Photo_SambaImpl.java
 * @brief 主に[設備写真データ管理]機能で使用する、ファイルサーバー通信に関する機能を実装するクラスを
 * 格納したファイル。
 **************************************************************************************
 */ 
...
/** 
 **************************************************************************************
 * @brief 主に[設備写真データ管理]機能で使用する、ファイルサーバー通信に関する機能を実装するクラス
 * 
 * @details 
...
@Component
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class Facility_Photo_SambaImpl implements Facility_Photo_Samba{
   ...
   * - この機能においては、ファイルパスを組み立てる際に用いる拡張子は、[png]のみとする。
  ...
  public void fileInsertUpdate(File file, Integer serial_num) throws IOException {
    ...
    String dir = File_Path_Enum.Facility_Photo.getPath();
    String ext = Datatype_Enum.PHOTO.getExtension();
