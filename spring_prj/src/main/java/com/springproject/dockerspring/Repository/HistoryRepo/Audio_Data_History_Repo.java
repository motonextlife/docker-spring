 ...
 * 
 * @brief データベースにアクセスするリポジトリのうち、[履歴情報の操作]に用いるリポジトリを格納した
 * パッケージ
 * 
 * @details
 * - このパッケージでは、データベースに保存されている履歴情報を、検索期間を指定して取得したり
 * 削除したりする機能を格納する。
 **************************************************************************************
 */ 
package com.springproject.dockerspring.Repository.HistoryRepo;
...
/** 
 **************************************************************************************
 * @file Audio_Data_History_Repo.java
 * @brief 主に[音源データ変更履歴]機能で使用する、データベースとの通信を行うリポジトリの
 ...
 * 
 * @details 
 * - このリポジトリが対応するデータベース内のテーブルの名称は[Audio_Data_History]である。
 ...
 **************************************************************************************
 */ 
public interface Audio_Data_History_Repo extends FindAllCrudRepository<Audio_Data_History, Integer>{
   ...
   */
  @Override
  public default FindAllResult<Audio_Data_History> findAllBranch(FindAllParam param) throws ExecutionException, InterruptedException{
    ...
    if(form_enum.isEmpty() || order == null || max == null || limit == null || offset == null){
      throw new IllegalArgumentException("Error location [Audio_Data_History_Repo:findAllBranch(IllegalArgumentException)]");
    }


    CompletableFuture<List<Audio_Data_History>> async_result = null;
    CompletableFuture<Integer> async_count = null;
    List<Audio_Data_History> result = null;
    Integer count = null;
    ...
    try{
      switch(form_enum.get()){
        ...
        case Id:
          async_result = order ? findBySound_id(word, start_datetime, end_datetime, limit, offset) : 
                                 findBySound_idDESC(word, start_datetime, end_datetime, limit, offset);
          async_count = findBySound_idCount(word, start_datetime, end_datetime, max);
          break;
      ...
      throw new ExecutionException("Error location [Audio_Data_History_Repo:findAllBranch(ExecutionException)]" + "\n" + e, e);
    }catch(InterruptedException e){
      throw new InterruptedException("Error location [Audio_Data_History_Repo:findAllBranch](InterruptedException)" + "\n" + e);
    }

    return new FindAllResult<Audio_Data_History>(result, count);
  }
  ...
  @Query("SELECT COUNT(*) FROM" 
      + " (" 
        + " SELECT * FROM Audio_Data_History" 
   ...
   */   
  @Async("Repo")
  @Query("SELECT * FROM Audio_Data_History" 
      + " WHERE change_datetime BETWEEN :start_datetime AND :end_datetime" 
      + " ORDER BY change_datetime ASC, history_id ASC" 
      + " LIMIT :limit" 
      + " OFFSET :offset")
  CompletableFuture<List<Audio_Data_History>> findAllByDateBetween(@Param("start_datetime") Date start_datetime, 
                                                                   @Param("end_datetime") Date end_datetime, 
  ...
  @Query("SELECT * FROM Audio_Data_History" 
      ...
      + " LIMIT :limit" 
      + " OFFSET :offset")
  CompletableFuture<List<Audio_Data_History>> findAllByDateBetweenDESC(@Param("start_datetime") Date start_datetime, 
                                                                       @Param("end_datetime") Date end_datetime, 
                                                                       @Param("limit") Integer limit, 
                                                                       @Param("offset") Integer offset); 
   ...
   * - 非同期実行を可能とする。
   * - 引数として渡された検索結果最大値以上はカウントできないようになっている。
   * - 検索方法としては、[音源番号]は部分一致検索、[履歴日時]は範囲一致検索で行っている。
   * - 実際の処理は、アノテーションに記載したクエリ文字列によってデータベースに問い合わせを行うことで実現する。
   * 
   * @par 使用アノテーション
   * - @Async("Repo")
   ...
   * 
   * @par 処理の大まかな流れ
   * -# 副問問い合わせにより、指定の[履歴日時][音源番号]のデータを検索結果最大数まで選択。
   * -# 副問問い合わせの取得結果をカウントしたものを最終結果として出力する。
   **********************************************************************************************
   */
  @Async("Repo")
  @Query("SELECT COUNT(*) FROM" 
      ...
      + " )" 
      + " AS sub")
  CompletableFuture<Integer> findBySound_idCount(@Param("word") String word, 
   ...
   * - 非同期実行を可能とする。
   * - 引数として渡されたオフセット値の位置からデータ取得を行い、制限数に至ったら取得を中断する。
   * - 並び替え対象のカラムは、優先的に[音源番号]で行い、その次に[履歴日時]で行う。
   * - 検索方法としては、[音源番号]は部分一致検索、[履歴日時]は範囲一致検索で行っている。
   * - 実際の処理は、アノテーションに記載したクエリ文字列によってデータベースに問い合わせを行うことで実現する。
   * - 最後に必ず[履歴番号]でソートを行う。
   * 
   ...
   * 
   * @par 処理の大まかな流れ
   * -# 指定の[履歴日時][音源番号]で、オフセットの範囲内のデータを出力する。
   **********************************************************************************************
   */    
  @Async("Repo")
  @Query("SELECT * FROM Audio_Data_History" 
      + " WHERE sound_id LIKE CONCAT('%', :word, '%')" 
        + " AND change_datetime BETWEEN :start_datetime AND :end_datetime" 
      + " ORDER BY sound_id ASC, change_datetime ASC, history_id ASC" 
      + " LIMIT :limit" 
      + " OFFSET :offset")
  CompletableFuture<List<Audio_Data_History>> findBySound_id(@Param("word") String word, 
                                                             @Param("start_datetime") Date start_datetime, 
                                                             @Param("end_datetime") Date end_datetime, 
                                                             @Param("limit") Integer limit, 
                                                             @Param("offset") Integer offset);
  ...
  @Query("SELECT * FROM Audio_Data_History" 
      + " WHERE sound_id LIKE CONCAT('%', :word, '%')" 
        + " AND change_datetime BETWEEN :start_datetime AND :end_datetime" 
      + " ORDER BY sound_id DESC, change_datetime DESC, history_id DESC" 
      + " LIMIT :limit" 
      + " OFFSET :offset")
  CompletableFuture<List<Audio_Data_History>> findBySound_idDESC(@Param("word") String word, 
                                                                 @Param("start_datetime") Date start_datetime, 
                                                                 @Param("end_datetime") Date end_datetime, 
                                                                 @Param("limit") Integer limit, 
                                                                 @Param("offset") Integer offset); 
  ...
  @Query("SELECT * FROM Audio_Data_History" 
      ...
      + " LIMIT :limit" 
      + " OFFSET :offset")
  CompletableFuture<List<Audio_Data_History>> findByChange_kinds(@Param("word") String word, 
                                                                 @Param("start_datetime") Date start_datetime, 
                                                                 @Param("end_datetime") Date end_datetime, 
                                                                 @Param("limit") Integer limit, 
                                                                 @Param("offset") Integer offset);
  ...
  @Query("SELECT * FROM Audio_Data_History" 
      ...
      + " LIMIT :limit" 
      + " OFFSET :offset")
  CompletableFuture<List<Audio_Data_History>> findByChange_kindsDESC(@Param("word") String word, 
                                                                     @Param("start_datetime") Date start_datetime, 
                                                                     @Param("end_datetime") Date end_datetime, 
                                                                     @Param("limit") Integer limit, 
                                                                     @Param("offset") Integer offset); 
  ...
  @Query("SELECT * FROM Audio_Data_History" 
      ...
      + " LIMIT :limit" 
      + " OFFSET :offset")
  CompletableFuture<List<Audio_Data_History>> findByOperation_user(@Param("word") String word, 
                                                                   @Param("start_datetime") Date start_datetime, 
                                                                   @Param("end_datetime") Date end_datetime, 
                                                                   @Param("limit") Integer limit, 
                                                                   @Param("offset") Integer offset);
  ...
  @Query("SELECT * FROM Audio_Data_History" 
      ...
      + " LIMIT :limit" 
      + " OFFSET :offset")
  CompletableFuture<List<Audio_Data_History>> findByOperation_userDESC(@Param("word") String word, 
                                                                       @Param("start_datetime") Date start_datetime, 
                                                                       @Param("end_datetime") Date end_datetime, 
                                                                       @Param("limit") Integer limit, 
                                                                       @Param("offset") Integer offset);
