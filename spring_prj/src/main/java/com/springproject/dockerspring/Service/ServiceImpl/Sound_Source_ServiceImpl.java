/** 
 **************************************************************************************
 * @file Sound_Source_ServiceImpl.java
 * @brief 主に[音源管理機能]が使用するサービスの機能を実装するクラスを格納するファイル。
 **************************************************************************************
 */ 
...
/** 
 **************************************************************************************
 * @brief 主に[音源管理機能]が使用するサービスの機能を実装するクラス
 * 
 * @details 
...
@Transactional(rollbackFor = Exception.class)
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class Sound_Source_ServiceImpl implements Sound_Source_Service{

  private final CommonUtilsService common_utils;
  private final Sound_Source_Repo normal_repo;
  private final Sound_Source_History_Repo hist_repo;
  private final Sound_Source_Pdf pdf_comp;
  private final Sound_Source_Csv csv_comp;
  private final Sound_Source_Csv_Form csv_form;
  private final Environment_Config config;
  private final Sound_Source_Form_Output_Validator validator;
   ...
   */
  @Override
  public Sound_Source selectData(String id) throws Exception, DataEmptyException, OutputDataFailedException {

    try{
      Sound_Source entity = this.normal_repo.findBySound_Id(id)
      ...
      throw new OutputDataFailedException("sound source failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Sound_Source_ServiceImpl:selectData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public void changeData(Sound_Source_Form form, History_Kind_Enum hist_kind) throws Exception {

    try{
      Sound_Source entity = this.common_utils.changeDatabaseCom(this.normal_repo, new Sound_Source(form));
      this.common_utils.changeDatabaseCom(this.hist_repo, new Sound_Source_History(entity, 
                                                                                   hist_kind, 
                                                                                   this.common_utils.getLoginUser(), 
                                                                                   new Date()));
    }catch(Exception e){
      throw new Exception("Error location [Sound_Source_ServiceImpl:changeData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public FindAllResult<Sound_Source> selectAllData(Com_Search_Form form) throws Exception, DataEmptyException, OutputDataFailedException{

    try{
      FindAllResult<Sound_Source> result = this.common_utils.selectAllDataCom(this.normal_repo, form);
      ...
      throw new Exception("Error location [Sound_Source_ServiceImpl:selectAllData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public FindAllResult<Sound_Source_History> selectHist(Com_Hist_Get_Form form) throws Exception, DataEmptyException, OutputDataFailedException{

    try{
      FindAllResult<Sound_Source_History> result = this.common_utils.selectHistCom(this.hist_repo, form);

      Predicate<Sound_Source_History> append_func = s -> {
      ...
      throw new OutputDataFailedException("sound source history failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Sound_Source_ServiceImpl:selectHist]" + "\n" + e);
    }
  }
  ...
  public void rollbackData(Rollback_Form form) throws Exception, DataEmptyException {
    ...
    try{
      Predicate<Sound_Source_History> new_append_check = s -> {
        return this.normal_repo.findBySound_Id(s.getSound_id()).isPresent();
      };

      Predicate<Sound_Source_History> latest_serial_check = s -> {
        Optional<Sound_Source> normal_ent = this.normal_repo.findById(s.getSerial_num());
    
        return normal_ent.isPresent() 
               && normal_ent.get().getSound_id().equals(s.getSound_id());
      };

      Consumer<Sound_Source_History> serial_latest = FuncWrap.throwConsumer(s -> {
        Sound_Source latest = this.normal_repo.findBySound_Id(s.getSound_id())
                                              .orElseThrow(() -> new DataEmptyException());

        s.setSerial_num(latest.getSerial_num());
      });

      Predicate<Sound_Source_History> rollback_output_valid_normal = o -> {
        Predicate<Sound_Source_History> append_func = s -> {
          return this.common_utils.outputValidHistInfoCom(s.getHistory_id(), 
                                                          s.getChange_datetime(), 
                                                          s.getChange_kinds(), 
      ...
      ManyParam<Sound_Source, Sound_Source_History> many_param = ManyParam.
        <Sound_Source, Sound_Source_History>builder()
        .rollback_form(form)
        ...
        .new_serial_out(Sound_Source::getSerial_num)
        .new_hist_id_out(Sound_Source_History::getHistory_id)
        .normal_entity_refill(s -> new Sound_Source(s))
        .hist_entity_refill(s -> new Sound_Source_History(s, History_Kind_Enum.ROLLBACK, 
                                                          this.common_utils.getLoginUser(), 
                                                          new Date()))
      ...
      throw new OutputDataFailedException("sound source history rollback failed" + "\n" + e);
    }catch (Exception e) {
      throw new Exception("Error location [Sound_Source_ServiceImpl:rollbackData]" + "\n" + e);
    }
  }
  ...
  public Map<String, String> parseString(EntitySetUp entity) throws Exception {
    ...
    try {
      ...
      throw new Exception("Error location [Sound_Source_ServiceImpl:parseString]" + "\n" + e);
    }
  }
  ...
  public List<Map<String, String>> mapPacking(List<EntitySetUp> data_list) throws Exception {
    ...
    try {
      ...
      throw new Exception("Error location [Sound_Source_ServiceImpl:mapPacking]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputPdf(String id) throws Exception, DataEmptyException, OutputDataFailedException {
    ...
    try{
      ...
      return new FileEntity(this.config.getSound_source_pdf() + "." + Datatype_Enum.PDF.getExtension(), file);
      ...
      throw new Exception("Error location [Sound_Source_ServiceImpl:outputPdf]" + "\n" + e);
    }
  }
  ...
  public CsvCheckResult inputCsv(Common_Csv_Form form) throws Exception, InputFileDifferException{
    ...
    try{
      Consumer<List<Sound_Source>> goto_database = FuncWrap.throwConsumer(list -> {
        List<Sound_Source> entities = (List<Sound_Source>)this.normal_repo.saveAll(list);

        List<Sound_Source_History> hist_list = 
          entities.parallelStream()
                  .map(s -> new Sound_Source_History(s, History_Kind_Enum.INSERT, 
      ...
      throw new InputFileDifferException("sound source input csv failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Sound_Source_ServiceImpl:inputCsv]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputCsv(Com_Search_Form form) throws Exception, DataEmptyException, OutputDataFailedException {
    ...
    try{
      Function<Integer, List<Map<String, String>>> generate = FuncWrap.throwFunction(i -> {
        Predicate<Sound_Source> valid_func = s -> {
          return this.common_utils.outputValidCom(s, this.validator, null, null);
        };

        FindAllResult<Sound_Source> result = this.common_utils.outputCsvCom(this.normal_repo, form, i, valid_func);
      ...
      return new FileEntity(this.config.getSound_source_csv() + "." + Datatype_Enum.CSV.getExtension(), file);
      ...
      throw new Exception("Error location [Sound_Source_ServiceImpl:outputCsv]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputCsvTemplate() throws Exception, OutputDataFailedException {
    ...
    try{
      ...
      throw new Exception("Error location [Sound_Source_ServiceImpl:outputCsvTemplate]" + "\n" + e);
    }
  }
  ...
  public void deleteData(Delete_Form form) throws Exception, DataEmptyException {
    ...
    try{
    Sound_Source entity = this.normal_repo.findById(form.getSerial_num_list().get(0).getSerial_num())
                                          .orElseThrow(() -> new DataEmptyException());

    this.common_utils.deleteDatabaseCom(this.normal_repo, entity);
    this.common_utils.changeDatabaseCom(this.hist_repo, new Sound_Source_History(entity, History_Kind_Enum.DELETE, 
                                                                                 this.common_utils.getLoginUser(), new Date()));

    }catch(DataEmptyException e){
      throw new DataEmptyException();
    }catch(Exception e){
      throw new Exception("Error location [Sound_Source_ServiceImpl:deleteData]" + "\n" + e);
    }
  }
  ...
  public CompletableFuture<Void> deleteHistory(Del_Indivi_Hist_Form form) throws Exception{
    ...
    try {
      ...
      throw new Exception("Error location [Sound_Source_ServiceImpl:deleteHistory]" + "\n" + e);
    }
  }
}