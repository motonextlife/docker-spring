/** 
 **************************************************************************************
 * @file Facility_ServiceImpl.java
 * @brief 主に[設備管理機能]が使用するサービスの機能を実装するクラスを格納するファイル。
 **************************************************************************************
 */ 
...
/** 
 **************************************************************************************
 * @brief 主に[設備管理機能]が使用するサービスの機能を実装するクラス
 * 
 * @details 
...
public class Facility_ServiceImpl implements Facility_Service{
  ...
  private final Facility_Repo normal_repo;
  private final Facility_History_Repo hist_repo;
  private final Facility_Pdf pdf_comp;
  private final Facility_Csv csv_comp;
  private final Facility_Csv_Form csv_form;
  private final Environment_Config config;
  private final Facility_Form_Output_Validator validator;
   ...
   */
  @Override
  public Facility selectData(String id) throws Exception, DataEmptyException, OutputDataFailedException {

    try{
      Facility entity = this.normal_repo.findByFaci_Id(id)
      ...
      throw new OutputDataFailedException("facility failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Facility_ServiceImpl:selectData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public void changeData(Facility_Form form, History_Kind_Enum hist_kind) throws Exception {

    try{
      Facility entity = this.common_utils.changeDatabaseCom(this.normal_repo, new Facility(form));
      this.common_utils.changeDatabaseCom(this.hist_repo, new Facility_History(entity, 
                                                                               hist_kind, 
                                                                               this.common_utils.getLoginUser(), 
                                                                               new Date()));
    }catch(Exception e){
      throw new Exception("Error location [Facility_ServiceImpl:changeData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public FindAllResult<Facility> selectAllData(Com_Search_Form form) throws Exception, DataEmptyException, OutputDataFailedException{

    try{
      FindAllResult<Facility> result = this.common_utils.selectAllDataCom(this.normal_repo, form);
      ...
      throw new Exception("Error location [Facility_ServiceImpl:selectAllData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public FindAllResult<Facility_History> selectHist(Com_Hist_Get_Form form) throws Exception, DataEmptyException, OutputDataFailedException{

    try{
      FindAllResult<Facility_History> result = this.common_utils.selectHistCom(this.hist_repo, form);

      Predicate<Facility_History> append_func = s -> {
      ...
      throw new OutputDataFailedException("facility history failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Facility_ServiceImpl:selectHist]" + "\n" + e);
    }
  }
  ...
  public void rollbackData(Rollback_Form form) throws Exception, DataEmptyException {
    ...
    try{
      Predicate<Facility_History> new_append_check = s -> {
        return this.normal_repo.findByFaci_Id(s.getFaci_id()).isPresent();
      };

      Predicate<Facility_History> latest_serial_check = s -> {
        Optional<Facility> normal_ent = this.normal_repo.findById(s.getSerial_num());
    
        return normal_ent.isPresent() 
               && normal_ent.get().getFaci_id().equals(s.getFaci_id());
      };

      Consumer<Facility_History> serial_latest = FuncWrap.throwConsumer(s -> {
        Facility latest = this.normal_repo.findByFaci_Id(s.getFaci_id())
                                          .orElseThrow(() -> new DataEmptyException());

        s.setSerial_num(latest.getSerial_num());
      });

      Predicate<Facility_History> rollback_output_valid_normal = o -> {
        Predicate<Facility_History> append_func = s -> {
          return this.common_utils.outputValidHistInfoCom(s.getHistory_id(), 
                                                          s.getChange_datetime(), 
                                                          s.getChange_kinds(), 
      ...
      ManyParam<Facility, Facility_History> many_param = ManyParam.
        <Facility, Facility_History>builder()
        .rollback_form(form)
        ...
        .new_serial_out(Facility::getSerial_num)
        .new_hist_id_out(Facility_History::getHistory_id)
        .normal_entity_refill(s -> new Facility(s))
        .hist_entity_refill(s -> new Facility_History(s, History_Kind_Enum.ROLLBACK, 
                                                      this.common_utils.getLoginUser(), 
                                                      new Date()))
      ...
      throw new OutputDataFailedException("facility history rollback failed" + "\n" + e);
    }catch (Exception e) {
      throw new Exception("Error location [Facility_ServiceImpl:rollbackData]" + "\n" + e);
    }
  }
  ...
  public Map<String, String> parseString(EntitySetUp entity) throws Exception {
    ...
    try {
      ...
      throw new Exception("Error location [Facility_ServiceImpl:parseString]" + "\n" + e);
    }
  }
  ...
  public List<Map<String, String>> mapPacking(List<EntitySetUp> data_list) throws Exception {
    ...
    try {
      ...
      throw new Exception("Error location [Facility_ServiceImpl:mapPacking]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputPdf(String id) throws Exception, DataEmptyException, OutputDataFailedException {
    ...
    try{
      ...
      return new FileEntity(this.config.getFacility_pdf() + "." + Datatype_Enum.PDF.getExtension(), file);
      ...
      throw new Exception("Error location [Facility_ServiceImpl:outputPdf]" + "\n" + e);
    }
  }
  ...
  public CsvCheckResult inputCsv(Common_Csv_Form form) throws Exception, InputFileDifferException{
    ...
    try{
      Consumer<List<Facility>> goto_database = FuncWrap.throwConsumer(list -> {
        List<Facility> entities = (List<Facility>)this.normal_repo.saveAll(list);

        List<Facility_History> hist_list = 
          entities.parallelStream()
                  .map(s -> new Facility_History(s, History_Kind_Enum.INSERT, 
      ...
      throw new InputFileDifferException("facility input csv failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Facility_ServiceImpl:inputCsv]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputCsv(Com_Search_Form form) throws Exception, DataEmptyException, OutputDataFailedException {
    ...
    try{
      Function<Integer, List<Map<String, String>>> generate = FuncWrap.throwFunction(i -> {
        Predicate<Facility> valid_func = s -> {
          return this.common_utils.outputValidCom(s, this.validator, null, null);
        };

        FindAllResult<Facility> result = this.common_utils.outputCsvCom(this.normal_repo, form, i, valid_func);
      ...
      return new FileEntity(this.config.getFacility_csv() + "." + Datatype_Enum.CSV.getExtension(), file);
      ...
      throw new Exception("Error location [Facility_ServiceImpl:outputCsv]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputCsvTemplate() throws Exception, OutputDataFailedException {
    ...
    try{
      ...
      throw new Exception("Error location [Facility_ServiceImpl:outputCsvTemplate]" + "\n" + e);
    }
  }
  ...
  public void deleteData(Delete_Form form) throws Exception, DataEmptyException {
    ...
    try{
    Facility entity = this.normal_repo.findById(form.getSerial_num_list().get(0).getSerial_num())
                                      .orElseThrow(() -> new DataEmptyException());

    this.common_utils.deleteDatabaseCom(this.normal_repo, entity);
    this.common_utils.changeDatabaseCom(this.hist_repo, new Facility_History(entity, History_Kind_Enum.DELETE, 
                                                                             this.common_utils.getLoginUser(), new Date()));

    }catch(DataEmptyException e){
      throw new DataEmptyException();
    }catch(Exception e){
      throw new Exception("Error location [Facility_ServiceImpl:deleteData]" + "\n" + e);
    }
  }
  ...
  public CompletableFuture<Void> deleteHistory(Del_Indivi_Hist_Form form) throws Exception{
    ...
    try {
      ...
      throw new Exception("Error location [Facility_ServiceImpl:deleteHistory]" + "\n" + e);
    }
  }
}