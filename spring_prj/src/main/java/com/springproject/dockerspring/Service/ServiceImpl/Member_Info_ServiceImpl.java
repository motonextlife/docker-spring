/** 
 **************************************************************************************
 * @file Member_Info_ServiceImpl.java
 * @brief 主に[団員管理機能]が使用するサービスの機能を実装するクラスを格納するファイル。
 **************************************************************************************
 */ 
...
/** 
 **************************************************************************************
 * @brief 主に[団員管理機能]が使用するサービスの機能を実装するクラス
 * 
 * @details 
...
@Transactional(rollbackFor = Exception.class)
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class Member_Info_ServiceImpl implements Member_Info_Service{

  private final CommonUtilsService common_utils;
  private final Member_Info_Repo normal_repo;
  private final Member_Info_History_Repo hist_repo;
  private final Member_Info_Pdf pdf_comp;
  private final Member_Info_Csv csv_comp;
  private final Member_Info_Csv_Form csv_form;
  private final Environment_Config config;
  private final Member_Info_Form_Output_Validator validator;
   ...
   */
  @Override
  public Member_Info selectData(String id) throws Exception, DataEmptyException, OutputDataFailedException {

    try{
      Member_Info entity = this.normal_repo.findByMember_id(id)
      ...
      throw new OutputDataFailedException("member info failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Member_Info_ServiceImpl:selectData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public void changeData(Member_Info_Form form, History_Kind_Enum hist_kind) throws Exception {

    try{
      Member_Info entity = this.common_utils.changeDatabaseCom(this.normal_repo, new Member_Info(form));
      this.common_utils.changeDatabaseCom(this.hist_repo, new Member_Info_History(entity, 
                                                                                  hist_kind, 
                                                                                  this.common_utils.getLoginUser(), 
                                                                                  new Date()));
    }catch(Exception e){
      throw new Exception("Error location [Member_Info_ServiceImpl:changeData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public FindAllResult<Member_Info> selectAllData(Com_Search_Form form) throws Exception, DataEmptyException, OutputDataFailedException{

    try{
      FindAllResult<Member_Info> result = this.common_utils.selectAllDataCom(this.normal_repo, form);
      ...
      throw new Exception("Error location [Member_Info_ServiceImpl:selectAllData]" + "\n" + e);
    }
  }
   ...
   */
  @Override
  public FindAllResult<Member_Info_History> selectHist(Com_Hist_Get_Form form) throws Exception, DataEmptyException, OutputDataFailedException{

    try{
      FindAllResult<Member_Info_History> result = this.common_utils.selectHistCom(this.hist_repo, form);

      Predicate<Member_Info_History> append_func = s -> {
      ...
      throw new OutputDataFailedException("member info history failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Member_Info_ServiceImpl:selectHist]" + "\n" + e);
    }
  }
  ...
  public void rollbackData(Rollback_Form form) throws Exception, DataEmptyException {
    ...
    try{
      Predicate<Member_Info_History> new_append_check = s -> {
        return this.normal_repo.findByMember_id(s.getMember_id()).isPresent();
      };

      Predicate<Member_Info_History> latest_serial_check = s -> {
        Optional<Member_Info> normal_ent = this.normal_repo.findById(s.getSerial_num());
    
        return normal_ent.isPresent() 
               && normal_ent.get().getMember_id().equals(s.getMember_id());
      };

      Consumer<Member_Info_History> serial_latest = FuncWrap.throwConsumer(s -> {
        Member_Info latest = this.normal_repo.findByMember_id(s.getMember_id())
                                             .orElseThrow(() -> new DataEmptyException());

        s.setSerial_num(latest.getSerial_num());
      });

      Predicate<Member_Info_History> rollback_output_valid_normal = o -> {
        Predicate<Member_Info_History> append_func = s -> {
          return this.common_utils.outputValidHistInfoCom(s.getHistory_id(), 
                                                          s.getChange_datetime(), 
                                                          s.getChange_kinds(), 
      ...
      ManyParam<Member_Info, Member_Info_History> many_param = ManyParam.
        <Member_Info, Member_Info_History>builder()
        .rollback_form(form)
        ...
        .new_serial_out(Member_Info::getSerial_num)
        .new_hist_id_out(Member_Info_History::getHistory_id)
        .normal_entity_refill(s -> new Member_Info(s))
        .hist_entity_refill(s -> new Member_Info_History(s, History_Kind_Enum.ROLLBACK, 
                                                         this.common_utils.getLoginUser(), 
                                                         new Date()))
      ...
      throw new OutputDataFailedException("member info history rollback failed" + "\n" + e);
    }catch (Exception e) {
      throw new Exception("Error location [Member_Info_ServiceImpl:rollbackData]" + "\n" + e);
    }
  }
  ...
  public Map<String, String> parseString(EntitySetUp entity) throws Exception {
    ...
    try {
      ...
      throw new Exception("Error location [Member_Info_ServiceImpl:parseString]" + "\n" + e);
    }
  }
  ...
  public List<Map<String, String>> mapPacking(List<EntitySetUp> data_list) throws Exception {
    ...
    try {
      ...
      throw new Exception("Error location [Member_Info_ServiceImpl:mapPacking]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputPdf(String id) throws Exception, DataEmptyException, OutputDataFailedException {
    ...
    try{
      ...
      return new FileEntity(this.config.getMember_info_pdf() + "." + Datatype_Enum.PDF.getExtension(), file);
      ...
      throw new Exception("Error location [Member_Info_ServiceImpl:outputPdf]" + "\n" + e);
    }
  }
  ...
  public CsvCheckResult inputCsv(Common_Csv_Form form) throws Exception, InputFileDifferException{
    ...
    try{
      Consumer<List<Member_Info>> goto_database = FuncWrap.throwConsumer(list -> {
        List<Member_Info> entities = (List<Member_Info>)this.normal_repo.saveAll(list);

        List<Member_Info_History> hist_list = 
          entities.parallelStream()
                  .map(s -> new Member_Info_History(s, History_Kind_Enum.INSERT, 
      ...
      throw new InputFileDifferException("member info input csv failed" + "\n" + e);
    }catch(Exception e){
      throw new Exception("Error location [Member_Info_ServiceImpl:inputCsv]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputCsv(Com_Search_Form form) throws Exception, DataEmptyException, OutputDataFailedException {
    ...
    try{
      Function<Integer, List<Map<String, String>>> generate = FuncWrap.throwFunction(i -> {
        Predicate<Member_Info> valid_func = s -> {
          return this.common_utils.outputValidCom(s, this.validator, null, null);
        };

        FindAllResult<Member_Info> result = this.common_utils.outputCsvCom(this.normal_repo, form, i, valid_func);
      ...
      return new FileEntity(this.config.getMember_info_csv() + "." + Datatype_Enum.CSV.getExtension(), file);
      ...
      throw new Exception("Error location [Member_Info_ServiceImpl:outputCsv]" + "\n" + e);
    }
  }
  ...
  public FileEntity outputCsvTemplate() throws Exception, OutputDataFailedException {
    ...
    try{
      ...
      throw new Exception("Error location [Member_Info_ServiceImpl:outputCsvTemplate]" + "\n" + e);
    }
  }
  ...
  public void deleteData(Delete_Form form) throws Exception, DataEmptyException {
    ...
    try{
    Member_Info entity = this.normal_repo.findById(form.getSerial_num_list().get(0).getSerial_num())
                                         .orElseThrow(() -> new DataEmptyException());

    this.common_utils.deleteDatabaseCom(this.normal_repo, entity);
    this.common_utils.changeDatabaseCom(this.hist_repo, new Member_Info_History(entity, History_Kind_Enum.DELETE, 
                                                                                this.common_utils.getLoginUser(), new Date()));

    }catch(DataEmptyException e){
      throw new DataEmptyException();
    }catch(Exception e){
      throw new Exception("Error location [Member_Info_ServiceImpl:deleteData]" + "\n" + e);
    }
  }
  ...
  public CompletableFuture<Void> deleteHistory(Del_Indivi_Hist_Form form) throws Exception{
    ...
    try {
      ...
      throw new Exception("Error location [Member_Info_ServiceImpl:deleteHistory]" + "\n" + e);
    }
  }
}