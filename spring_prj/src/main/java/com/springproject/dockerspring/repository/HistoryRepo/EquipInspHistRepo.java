...
/**************************************************************/
/*   [EquipInspHistRepo]
/*   テーブル「Equip_Inspect_History」へのデータベース
/*   アクセスを行う。
/**************************************************************/
public interface EquipInspHistRepo extends CrudRepository<Equip_Inspect_History, Integer>{
  ...
  /******************************************************************************************/
  @Query(
    "SELECT * FROM Equip_Inspect_History " + 
    "WHERE inspect_id = :inspect_id AND change_datetime BETWEEN :start_datetime AND :end_datetime LIMIT 500"
  )
  Iterable<Equip_Inspect_History> findAllByDateBetween(
    @Param("inspect_id") String inspect_id, 
  ...
  @Modifying
  @Query(
    "DELETE FROM Equip_Inspect_History " + 
