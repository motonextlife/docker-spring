...
/**************************************************************/
/*   [RecEvalRecordHistRepo]
/*   テーブル「Rec_Eval_Record_History」へのデータベース
/*   アクセスを行う。
/**************************************************************/
public interface RecEvalRecordHistRepo extends CrudRepository<Rec_Eval_Record_History, Integer>{
  ...
  /******************************************************************************************/
  @Query(
    "SELECT history_id, change_datetime, change_kinds, operation_user, serial_num, eval_id, data_kinds, record_name " + 
    "FROM Rec_Eval_Record_History " + 
    "WHERE eval_id = :eval_id AND change_datetime BETWEEN :start_datetime AND :end_datetime LIMIT 500"
  )
  Iterable<Rec_Eval_Record_History> findAllByDateBetween(
    @Param("eval_id") String evalsheet_id, 
  ...
  @Modifying
  @Query(
    "DELETE FROM Rec_Eval_Record_History " + 
