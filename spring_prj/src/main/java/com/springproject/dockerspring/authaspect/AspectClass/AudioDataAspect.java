...
/**************************************************************/
/*   [AudioDataAspect]
/*   「音源データ管理機能」への、権限判断を行う。
/*   なお、判断対象のメソッド名は、末尾の「Auth」を除いて
/*   同一にしてある。
/**************************************************************/
@Component
@Aspect
public class AudioDataAspect extends CommonAspect implements ComBlobAuth{

	//対象コントローラーの定義
	@Pointcut("com.springproject.dockerspring.controller.BlobController.AudioDataManage")
	...
	public Object selectBlobHistroyAuth(ProceedingJoinPoint jp) {
		return checkAuth(jp, Usage_Authority_Enum.Sound_Source_Hist_Brows);
	}
	...
	public Object deleteBlobDataAuth(ProceedingJoinPoint jp) {
		return checkAuth(jp, Usage_Authority_Enum.Sound_Source_Delete);
	}
