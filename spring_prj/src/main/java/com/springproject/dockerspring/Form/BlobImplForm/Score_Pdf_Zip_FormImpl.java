/** 
 **************************************************************************************
 * @file Score_Pdf_Zip_FormImpl.java
 * @brief 主に[楽譜データ管理]機能で使用する、ZIPから取り出したバイナリデータをバリデーション
 * するクラスを格納するファイル。
 **************************************************************************************
 */ 
...
@Component
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class Score_Pdf_Zip_FormImpl implements Score_Pdf_Zip_Form{
  ...
  public void blobChecker(List<FileEntity> input_list) throws InputFileDifferException, IOException {
    ...
    try{
      if(input_list.size() > this.config.getPdf_list_size()){
      ...
      throw new IOException("Error location [Score_Pdf_Zip_FormImpl:blobChecker]" + "\n" + e);
    }
  }
   ...
   * - ファイルチェックに関して、チェック基準は環境変数の値に従う。また、データ型に関しては[楽譜データ]の
   * 基準に従う。
   * 
  ...
  public void sequentialCheck(String filename, File file) throws InputFileDifferException, IOException {
    ...
    try(BufferedInputStream bis = new BufferedInputStream(new FileInputStream(file))){
      ...
      if(!ext_name.equals(Datatype_Enum.PDF.getExtension())){
        throw new InputFileDifferException("missing extension");
      }

      Tika tika = new Tika();
      String mime = tika.detect(bis);
      if(!mime.equals(Datatype_Enum.PDF.getMimetype())){
        throw new InputFileDifferException("missing mimetype");
      }

      if(file.length() > this.config.getPdf_limit()){
      ...
      throw new IOException("Error location [Score_Pdf_Zip_FormImpl:sequentialCheck]" + "\n" + e);
    }
  }
}