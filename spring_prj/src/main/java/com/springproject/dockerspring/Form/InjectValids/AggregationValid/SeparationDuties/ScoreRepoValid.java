/** 
 **************************************************************************************
 * @file ScoreRepoValid.java
 * @brief DIが必要なフォームバリデーションの項目に関して、[楽譜関連のリポジトリからの値が必要な項目]の
...
/** 
 **************************************************************************************
 * @brief DIが必要なフォームバリデーションの項目に関して、[楽譜関連のリポジトリからの値が必要な項目]
...
@Component
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class ScoreRepoValid{
  
  private final Musical_Score_Repo music_score_repo;
  private final Musical_Score_History_Repo music_score_hist;
  private final Score_Pdf_Repo score_pdf_repo;
  private final Score_Pdf_History_Repo score_pdf_hist;
  ...
  public Boolean historyIdExistCheck(Integer history_id, ValidTable_Enum table) {
    ...
    switch(table){
      case Musical_Score_History:
        judge = music_score_hist.existsById(history_id);
        break;

      case Score_Pdf_History:
        Optional<Score_Pdf_History> score_pdf_opt = score_pdf_hist.findById(history_id);
        judge = score_pdf_opt.isPresent() 
             && music_score_repo.findByScore_Id(score_pdf_opt.get().getScore_id()).isPresent();
        break;

      default:
        throw new IllegalArgumentException("Error location [ScoreRepoValid:historyIdExistCheck]");
    }
  ...
  public Boolean pairExistCheckValid(String id, List<Integer> serial_num_list, ValidTable_Enum table) {
    ...
    switch(table){
      case Musical_Score:
        Optional<Musical_Score> music_score_opt = music_score_repo.findById(serial_num_list.get(0));
        judge = music_score_opt.isPresent() && music_score_opt.get().getScore_id().equals(id);
        break;

      case Score_Pdf:
        List<Optional<Score_Pdf>> score_pdf_opt = serial_num_list.stream()
                                                                 .parallel()
                                                                 .map(s -> score_pdf_repo.findById(s))
                                                                 .toList();
        judge = score_pdf_opt.stream()
                             .parallel()
                             .allMatch(s -> s.isPresent() && s.get().getScore_id().equals(id));
        break;

      default:
        throw new IllegalArgumentException("Error location [ScoreRepoValid:pairExistCheckValid]");
    }
  ...
  public Boolean foreignKeyCheck(String id, ValidTable_Enum table) {
    ...
    switch(table){
      case Musical_Score:
        judge = music_score_repo.findByScore_Id(id).isPresent();
        break;

      default:
        throw new IllegalArgumentException("Error location [ScoreRepoValid:foreignKeyCheck]");
    }
  ...
  public Boolean idUniqueCheck(String id, Integer serial_num, ValidTable_Enum table) {
    ...
    switch(table){
      case Musical_Score:
        if(serial_num == null){
          judge = music_score_repo.findByScore_Id(id).isEmpty();
        }else{
          if(music_score_repo.findById(serial_num).isPresent()){
            judge = music_score_repo.checkUnique(serial_num, id).equals(0);
          }
        }
        break;

      default:
        throw new IllegalArgumentException("Error location [ScoreRepoValid:idUniqueCheck]");
    }
