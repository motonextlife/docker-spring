 ...
 * 
 * @brief [設備管理]に関するテストケース生成処理のうち、[データベース]
...
/** 
 **************************************************************************************
 * @file Facility_Database_TestCase.java
...
@Component
@Import({ConfigureFactory.class, History_TestCase_Make.class})
public class Facility_Database_TestCase{
  
  private final Facility_Database_Yaml database_yml;
   ...
   */ 
  @Autowired
  public Facility_Database_TestCase(ConfigureFactory config_factory, 
                                    History_TestCase_Make hist_testcase_make) throws IOException{

    try(InputStream in_database_yml = new ClassPathResource("TestCaseFile/Facility/facility-database.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.database_yml = yaml.loadAs(in_database_yml, Facility_Database_Yaml.class);
    }
    ...
    for(int i = 1; i <= 20; i++){
      Operation operation = Insert.into(NORMAL_TABLE_NAME)
                                  ...
                                  .values(i,
                                          ...
                                          this.database_yml.getFaci_name().get("case_" + i), 
                                          this.database_yml.getBuy_date().get("case_" + i), 
                                          this.database_yml.getProducer().get("case_" + i), 
  ...
  public Facility compareEntityMake(Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      Facility mock_entity = mock(Facility.class);
      ...
      when(mock_entity.getFaci_name()).thenReturn(this.database_yml.getFaci_name().get("case_" + testcase_num));
      when(mock_entity.getBuy_date()).thenReturn(this.database_yml.getBuy_date().get("case_" + testcase_num));
      when(mock_entity.getProducer()).thenReturn(this.database_yml.getProducer().get("case_" + testcase_num));
  ...
  public Facility UpdateAfterEntityMake(Integer testcase_num) throws ParseException{
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      when(mock_entity.getSerial_num()).thenReturn(testcase_num);
      when(mock_entity.getFaci_id()).thenReturn("id" + testcase_num);
      when(mock_entity.getFaci_name()).thenReturn("Blank");
      when(mock_entity.getBuy_date()).thenReturn(parse_date.parse("1996-2-19"));
      when(mock_entity.getProducer()).thenReturn("Blank");
  ...
  public Facility justInTimeEntityMake(){
    ...
    when(mock_entity.getFaci_name()).thenReturn(this.database_yml.getFaci_name().get("ok_length"));
    when(mock_entity.getBuy_date()).thenReturn(this.database_yml.getBuy_date().get("case_1"));
    when(mock_entity.getProducer()).thenReturn(this.database_yml.getProducer().get("ok_length"));
  ...
  public Facility okNullEntityMake(){
    ...
    when(mock_entity.getSerial_num()).thenReturn(1);
    when(mock_entity.getFaci_id()).thenReturn(this.database_yml.getFaci_id().get("ok_null_inspect"));
    when(mock_entity.getFaci_name()).thenReturn(this.database_yml.getFaci_name().get("case_1"));
    when(mock_entity.getBuy_date()).thenReturn(this.database_yml.getBuy_date().get("case_1"));
    when(mock_entity.getProducer()).thenReturn(this.database_yml.getProducer().get("case_1"));
  ...
  public List<Facility> failedEntityMake(){
    ...
    List<Facility> fail_mock_list = new ArrayList<>();
    int i = 1;

    String[] faci_id = {"ng_overflow", "ng_null"};
    for(String key_name: faci_id){
      Facility mock_entity = compareEntityMake(1);
    ...
    String[] faci_name = {"ng_overflow", "ng_null"};
    for(String key_name: faci_name){
      Facility mock_entity = compareEntityMake(1);
            
      when(mock_entity.getSerial_num()).thenReturn(i++);
      when(mock_entity.getFaci_name()).thenReturn(this.database_yml.getFaci_name().get(key_name));
    ...
    String[] producer = {"ng_overflow"};
    for(String key_name: producer){
      Facility mock_entity = compareEntityMake(1);
            
      when(mock_entity.getSerial_num()).thenReturn(i++);
      when(mock_entity.getProducer()).thenReturn(this.database_yml.getProducer().get(key_name));
  ...
  public Facility UniqueMissEntityMake(){
    ...
    when(mock_entity.getSerial_num()).thenReturn(1);
    when(mock_entity.getFaci_id()).thenReturn(this.database_yml.getFaci_id().get("ng_unique"));
    when(mock_entity.getFaci_name()).thenReturn(this.database_yml.getFaci_name().get("case_1"));
    when(mock_entity.getBuy_date()).thenReturn(this.database_yml.getBuy_date().get("case_1"));
    when(mock_entity.getProducer()).thenReturn(this.database_yml.getProducer().get("case_1"));
  ...
  public Facility_History compareHistoryEntityMake(Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      Facility_History mock_entity = mock(Facility_History.class);
  ...
  public List<Facility_History> failedHistoryEntityMake(){
    ...
    List<Facility_History> fail_mock_list = new ArrayList<>();
    int i = 0;

    for(Date fail_data: this.hist_testcase_make.getFailChange_datetime()){
      Facility_History mock_entity = compareHistoryEntityMake(1);
