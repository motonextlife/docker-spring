/** 
 **************************************************************************************
 * @package com.springproject.dockerspring.CommonTestCaseMaker.FacilityPhoto.FailDatabase
 * 
 * @brief [設備写真データ管理機能]に関するテストケース生成処理のうち、[出力失敗ケースのデータベース]
...
/** 
 **************************************************************************************
 * @file Facility_Photo_Fail_Database_TestCase.java
 * @brief 主に[設備写真データ情報]機能のテストにおいて、データベースとの通信テストの際に、あらかじめ
...
import com.springproject.dockerspring.CommonTestCaseMaker.FacilityPhoto.Form.Facility_Photo_Form_Yaml;
 ...
 * @see Facility_Photo_Fail_Samba_TestCase
 **************************************************************************************
 */ 
@Component
@Import({ConfigureFactory.class, History_TestCase_Make.class})
public class Facility_Photo_Fail_Database_TestCase{
  
  private final Facility_Photo_Form_Yaml form_yml;
  private final History_TestCase_Make hist_testcase_make;
  private final Facility_Photo_Fail_Samba_TestCase samba_fail_testcase;
  ...
  private final String NORMAL_TABLE_NAME = "Facility_Photo";
  private final String HISTORY_TABLE_NAME = "Facility_Photo_History";
   ...
   * @param[in] samba_fail_testcase [設備写真データ情報]のファイルサーバー関連のエラーテストケースクラス
   * 
   * @par 処理の大まかな流れ
   ...
   */ 
  @Autowired
  public Facility_Photo_Fail_Database_TestCase(ConfigureFactory config_factory, 
                                               History_TestCase_Make hist_testcase_make, 
                                               Facility_Photo_Fail_Samba_TestCase samba_fail_testcase) throws IOException{
  ...
  public Map<Integer, String> failDatabaseSetup(Boolean link_samba) throws IOException{
    ...
    List<Map<Facility_Photo_TestKeys, String>> fail_list = getDatabaseFail(link_samba);
    Map<Integer, String> search_map = new HashMap<>();

    int i = 1;
    for(Map<Facility_Photo_TestKeys, String> fail_map: fail_list){
      String photo_name = "";
      String photo_hash = "";

      if(link_samba){
        String filename = i + ".png";
        photo_name = fail_map.get(Facility_Photo_TestKeys.Photo_Data);
        photo_hash = this.samba_fail_testcase.failSambaSetup(photo_name, filename);
      }else{
        photo_name = fail_map.get(Facility_Photo_TestKeys.Photo_Name);
        photo_hash = "EnterSuitableString";
      }
      ...
      Operation operation = Insert.into(NORMAL_TABLE_NAME)
                                  .columns("serial_num",
                                          ...
                                          "photo_hash")
                                  .values(i,
                                          fail_map.get(Facility_Photo_TestKeys.Faci_Id), 
                                          photo_name, 
                                          photo_hash)
                                  .build();

      operation_list.add(operation);
      search_map.put(i++, fail_map.get(Facility_Photo_TestKeys.Faci_Id));
    }
  ...
  public List<Integer> historyFailDatabaseSetup(Boolean link_samba) throws IOException, ParseException{
    ...
    for(Map<String, String> fail_map: fail_list){
      ...
      if(link_samba){
        ...
        photo_name = fail_map.get("photo_data");
        photo_hash = this.samba_fail_testcase.historyFailSambaSetup(photo_name, filename);
      }else{
        photo_name = fail_map.get("photo_name");
        photo_hash = "EnterSuitableString";
      }
   ...
   * -# 設備番号が参照元テーブルの設備番号を参照できるように、「設備番号がエラーのケース」以外のエラーケースに
   * 関しては、確実に参照できてエラーが起こらない設備番号に上書きする。
   * -# 取り出したデータは、順番にリストに格納する。
   * -# 各項目の格納が終わったリストを全て結合し、一つのリストにしたものを戻り値とする。
   **********************************************************************************************
   */ 
  private List<Map<Facility_Photo_TestKeys, String>> getDatabaseFail(Boolean link_samba){
    List<Map<Facility_Photo_TestKeys, String>> error_list = new ArrayList<>();

    if(link_samba){
      error_list.addAll(makePhotoDataFailedData());
    }else{
      error_list.addAll(makePhotoNameFailedData());
    }

    for(Map<Facility_Photo_TestKeys, String> id_override: error_list){
      String faci_id = id_override.get(Facility_Photo_TestKeys.Faci_Id);
      faci_id = faci_id.replaceFirst("....$", "") + "_" + 1;

      id_override.put(Facility_Photo_TestKeys.Faci_Id, faci_id);
    }
  ...
  private List<Map<String, String>> getHistoryDatabaseFail(Boolean link_samba){
    ...
    List<Map<Facility_Photo_TestKeys, String>> normal_table = getDatabaseFail(link_samba);
    List<Map<History_TestKeys, String>> hist_table = this.hist_testcase_make.getHistoryDatabaseFailData();


    for(Map<Facility_Photo_TestKeys, String> normal_map: normal_table){
      ...
      strkey_map.put("faci_id", normal_map.get(Facility_Photo_TestKeys.Faci_Id));

      if(link_samba){
        strkey_map.put("photo_data", normal_map.get(Facility_Photo_TestKeys.Photo_Data));
      }else{
        strkey_map.put("photo_name", normal_map.get(Facility_Photo_TestKeys.Photo_Name));
      }
    ...
    for(Map<History_TestKeys, String> hist_map: hist_table){
      ...
      strkey_map.put("faci_id", makeBaseData().get(Facility_Photo_TestKeys.Faci_Id));

      if(link_samba){
        strkey_map.put("photo_data", makeBaseData().get(Facility_Photo_TestKeys.Photo_Data));
      }else{
        strkey_map.put("photo_name", makeBaseData().get(Facility_Photo_TestKeys.Photo_Name));
      }
   ...
   **********************************************************************************************
   */ 
  private Map<Facility_Photo_TestKeys, String> makeBaseData(){

    Map<Facility_Photo_TestKeys, String> base_map = new HashMap<>();
    String childkey = "ok";

    base_map.put(Facility_Photo_TestKeys.Faci_Id, this.form_yml.getFaci_id().get(childkey));
    base_map.put(Facility_Photo_TestKeys.Photo_Name, this.form_yml.getPhoto_name().get(childkey));
    base_map.put(Facility_Photo_TestKeys.Photo_Data, this.form_yml.getPhoto_data().get(childkey));
   ...
   **********************************************************************************************
   */ 
  private List<Map<Facility_Photo_TestKeys, String>> makeFaciIdFailedData(){
    ...
    for(String key: key_name){
      Map<Facility_Photo_TestKeys, String> base_data = makeBaseData();
      base_data.put(Facility_Photo_TestKeys.Faci_Id, this.form_yml.getFaci_id().get(key));
  ...
  /** 
   **********************************************************************************************
   * @brief 項目[設備写真データ名]でエラーになるテストケースを格納したマップリストを作成する。
   * 
   * @details
   * - マッピングしたエンティティから該当項目のエラーケースを取り出し、ベースとなる合格ケースのリストを
   * 上書きすることで作成する。
   ...
   **********************************************************************************************
   */ 
  private List<Map<Facility_Photo_TestKeys, String>> makePhotoNameFailedData(){
    ...
    for(String key: key_name){
      Map<Facility_Photo_TestKeys, String> base_data = makeBaseData();
      base_data.put(Facility_Photo_TestKeys.Photo_Name, this.form_yml.getPhoto_name().get(key));
      error_list.add(base_data);
    }
  ...
  /** 
   **********************************************************************************************
   * @brief 項目[設備写真データ]でエラーになるテストケースを格納したマップリストを作成する。
   * 
   * @details
   * - マッピングしたエンティティから該当項目のエラーケースを取り出し、ベースとなる合格ケースのリストを
   * 上書きすることで作成する。
   ...
   **********************************************************************************************
   */ 
  private List<Map<Facility_Photo_TestKeys, String>> makePhotoDataFailedData(){
    ...
    for(String key: key_name){
      Map<Facility_Photo_TestKeys, String> base_data = makeBaseData();
      base_data.put(Facility_Photo_TestKeys.Photo_Data, this.form_yml.getPhoto_data().get(key));
      error_list.add(base_data);
    }
