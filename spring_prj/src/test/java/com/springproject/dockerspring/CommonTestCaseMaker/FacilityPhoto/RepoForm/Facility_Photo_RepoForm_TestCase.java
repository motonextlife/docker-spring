 ...
 * 
 * @brief [設備写真データ管理機能]に関するテストケース生成処理のうち、[データベースのフリーワード検索]
...
/** 
 **************************************************************************************
 * @file Facility_Photo_RepoForm_TestCase.java
 * @brief 主に[設備写真データ管理]機能のテストにおいて、リポジトリの検索のテストを行う際の検索
 ...
 * - @Component
 * 
 * @see Facility_Photo_RepoForm_Yaml
 **************************************************************************************
 */ 
@Component
public class Facility_Photo_RepoForm_TestCase{
  
  private final Facility_Photo_RepoForm_Yaml repoform_yml;
  ...
  //! 画像ファイル名
  public static final Integer PHOTO_NAME = 2;
   ...
   **********************************************************************************************
   */ 
  public Facility_Photo_RepoForm_TestCase() throws IOException{

    try(InputStream in_repoform_yml = new ClassPathResource("TestCaseFile/Facilityphoto/facility-photo-repoform.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.repoform_yml = yaml.loadAs(in_repoform_yml, Facility_Photo_RepoForm_Yaml.class);
    }
  }
  ...
  public String[] otherRepoForm(Facility_Photo_TestKeys subject_key, Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      switch(subject_key){
        ...
        case Photo_Name:
          return this.repoform_yml.getPhoto_name().get("word_" + testcase_num);
        default:
          throw new IllegalArgumentException();
      }      
    }
