 ...
 * 
 * @brief [楽譜データ管理機能]に関するテストケース生成処理のうち、[データベースとファイルサーバー]
...
/** 
 **************************************************************************************
 * @file Score_Pdf_Database_TestCase.java
 ...
 * 
 * @see Score_Pdf_Database_Yaml
 * @see ConfigureFactory
 * @see History_TestCase_Make
 * @see Score_Pdf_Samba_TestCase
 **************************************************************************************
 */ 
@Component
@Import({ConfigureFactory.class, History_TestCase_Make.class})
public class Score_Pdf_Database_TestCase{
  
  private final Score_Pdf_Database_Yaml database_yml;
  private final History_TestCase_Make hist_testcase_make;
  private final Score_Pdf_Samba_TestCase samba_testcase;
   ...
   * @param[in] samba_testcase [楽譜データ情報]のファイルサーバー関連のテストケースクラス
   * 
   * @par 処理の大まかな流れ
   ...
   */ 
  @Autowired
  public Score_Pdf_Database_TestCase(ConfigureFactory config_factory, 
                                     History_TestCase_Make hist_testcase_make, 
                                     Score_Pdf_Samba_TestCase samba_testcase) throws IOException{

    try(InputStream in_database_yml = new ClassPathResource("TestCaseFile/ScorePdf/score-pdf-database.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.database_yml = yaml.loadAs(in_database_yml, Score_Pdf_Database_Yaml.class);
    }
   ...
   * -# 組み上げたクエリを実行する。
   * 
   * @note 特に指定はないが、ファイルサーバーへの保存時の拡張子は[pdf]とする。
   * 
   * @throw IOException
   **********************************************************************************************
   */ 
  public void databaseSetup(Boolean link_samba) throws IOException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      if(link_samba){
        ...
        score_name = this.database_yml.getPdf_data().get("case_" + i);
        pdf_hash = this.samba_testcase.sambaSetup(score_name, filename);
      }else{
        score_name = this.database_yml.getScore_name().get("case_" + i);
        pdf_hash = this.database_yml.getPdf_hash().get("case_" + i);
      }
  ...
  public Score_Pdf compareEntityMake(Integer testcase_num, Boolean link_samba) throws IOException{
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      Score_Pdf mock_entity = mock(Score_Pdf.class);
      ...
      when(mock_entity.getScore_name()).thenReturn(this.database_yml.getScore_name().get("case_" + testcase_num));

      if(link_samba){
        when(mock_entity.getPdf_hash()).thenReturn(makeHashString(testcase_num));
      }else{
        when(mock_entity.getPdf_hash()).thenReturn(this.database_yml.getPdf_hash().get("case_" + testcase_num));
      }
   ...
   * - ただし楽譜番号に関しては参照制約があり、適当な値が入れれないので、若干既存のテストケースと被る。
   * - 範囲外の数字が来たときは処理を続行できないのでエラーを投げる。
   * 
   * @param[in] testcase_num 指定するテストケースの番号
   * 
  ...
  public Score_Pdf UpdateAfterEntityMake(Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      when(mock_entity.getScore_name()).thenReturn("Blank");
      when(mock_entity.getPdf_hash()).thenReturn("Blank");
  ...
  public Score_Pdf justInTimeEntityMake(){
    ...
    when(mock_entity.getScore_name()).thenReturn(this.database_yml.getScore_name().get("ok_length"));
    when(mock_entity.getPdf_hash()).thenReturn(this.database_yml.getPdf_hash().get("ok_length"));
  ...
  public List<Score_Pdf> failedEntityMake() throws IOException{
    ...
    List<Score_Pdf> fail_mock_list = new ArrayList<>();
    int i = 1;

    String[] score_id = {"ng_overflow", "ng_foreign"};
    for(String key_name: score_id){
      Score_Pdf mock_entity = compareEntityMake(1, false);
    ...
    String[] score_name = {"ng_overflow", "ng_null"};
    for(String key_name: score_name){
      Score_Pdf mock_entity = compareEntityMake(1, false);
            
      when(mock_entity.getSerial_num()).thenReturn(i++);
      when(mock_entity.getScore_name()).thenReturn(this.database_yml.getScore_name().get(key_name));

      fail_mock_list.add(mock_entity);
    }

    String[] pdf_hash = {"ng_overflow", "ng_null"};
    for(String key_name: pdf_hash){
      Score_Pdf mock_entity = compareEntityMake(1, false);
            
      when(mock_entity.getSerial_num()).thenReturn(i++);
      when(mock_entity.getPdf_hash()).thenReturn(this.database_yml.getPdf_hash().get(key_name));
  ...
  public void historyDatabaseSetup(Boolean link_samba) throws IOException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      if(link_samba){
        ...
        score_name = this.database_yml.getPdf_data().get("case_" + i);
        pdf_hash = this.samba_testcase.historySambaSetup(score_name, filename);
      }else{
        score_name = this.database_yml.getScore_name().get("case_" + i);
        pdf_hash = this.database_yml.getPdf_hash().get("case_" + i);
      }
  ...
  public Score_Pdf_History compareHistoryEntityMake(Integer testcase_num, Boolean link_samba) throws IOException{
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      Score_Pdf_History mock_entity = mock(Score_Pdf_History.class);
  ...
  public List<Score_Pdf_History> failedHistoryEntityMake() throws IOException{
    ...
    List<Score_Pdf_History> fail_mock_list = new ArrayList<>();
    int i = 0;

    for(Date fail_data: this.hist_testcase_make.getFailChange_datetime()){
      Score_Pdf_History mock_entity = compareHistoryEntityMake(1, false);
  ...
  private String makeHashString(Integer testcase_num) throws IOException{
    ...
    String file_name = this.database_yml.getPdf_data().get("case_" + testcase_num);
    String file_path = "src/main/resources/TestCaseFile/ScorePdf/BlobTestCaseData/setup/" + file_name;
  ...
  public String compareSambaFileMake(Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      return this.database_yml.getPdf_data().get("case_" + testcase_num);
    }
  }
