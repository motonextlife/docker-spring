 ...
 * 
 * @brief [楽譜データ管理機能]に関するテストケース生成処理のうち、[ZIP圧縮ファイルの入出力]
...
/** 
 **************************************************************************************
 * @file Score_Pdf_Zip_TestCase.java
 * @brief 主に[楽譜データ機能]機能のテストにおいて、ZIPの入出力の機能において使用するテスト
 ...
 * - @Component
 * 
 * @see Score_Pdf_Zip_Yaml
 **************************************************************************************
 */ 
@Component
public class Score_Pdf_Zip_TestCase{
  
  private final Score_Pdf_Zip_Yaml zip_yml;
   ...
   **********************************************************************************************
   */ 
  public Score_Pdf_Zip_TestCase() throws IOException{

    try(InputStream in_zip_yml = new ClassPathResource("TestCaseFile/ScorePdf/score-pdf-zip.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.zip_yml = yaml.loadAs(in_zip_yml, Score_Pdf_Zip_Yaml.class);
    }
  }
  ...
  /** 
   **********************************************************************************************
   * @brief 項目[楽譜ZIPファイル]でバリデーションエラーになるテストケースを読み取って、リストを作成する。
   * 
   * @details
  ...
  public List<String> getZipFailedData(Boolean append_form){
    ...
    if(append_form){
      fail_list.add(this.zip_yml.getZip_file().get("ng_no_pdf_mid"));
      fail_list.add(this.zip_yml.getZip_file().get("ng_no_pdf_svg"));
      fail_list.add(this.zip_yml.getZip_file().get("ng_filename_overflow"));
      fail_list.add(this.zip_yml.getZip_file().get("ng_image"));
