 ...
 * 
 * @brief [設備写真データ管理機能]に関するテストケース生成処理のうち、[データベースとファイルサーバー]
...
/** 
 **************************************************************************************
 * @file Facility_Photo_Database_TestCase.java
 ...
 * 
 * @see Facility_Photo_Database_Yaml
 * @see ConfigureFactory
 * @see History_TestCase_Make
 * @see Facility_Photo_Samba_TestCase
 **************************************************************************************
 */ 
@Component
@Import({ConfigureFactory.class, History_TestCase_Make.class})
public class Facility_Photo_Database_TestCase{
  
  private final Facility_Photo_Database_Yaml database_yml;
  private final History_TestCase_Make hist_testcase_make;
  private final Facility_Photo_Samba_TestCase samba_testcase;
   ...
   * @param[in] samba_testcase [設備写真データ情報]のファイルサーバー関連のテストケースクラス
   * 
   * @par 処理の大まかな流れ
   ...
   */ 
  @Autowired
  public Facility_Photo_Database_TestCase(ConfigureFactory config_factory, 
                                          History_TestCase_Make hist_testcase_make, 
                                          Facility_Photo_Samba_TestCase samba_testcase) throws IOException{

    try(InputStream in_database_yml = new ClassPathResource("TestCaseFile/FacilityPhoto/facility-photo-database.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.database_yml = yaml.loadAs(in_database_yml, Facility_Photo_Database_Yaml.class);
    }
   ...
   * -# 組み上げたクエリを実行する。
   * 
   * @note 特に指定はないが、ファイルサーバーへの保存時の拡張子は[png]とする。
   * 
   * @throw IOException
   **********************************************************************************************
   */ 
  public void databaseSetup(Boolean link_samba) throws IOException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      if(link_samba){
        ...
        photo_name = this.database_yml.getPhoto_data().get("case_" + i);
        photo_hash = this.samba_testcase.sambaSetup(photo_name, filename);
      }else{
        photo_name = this.database_yml.getPhoto_name().get("case_" + i);
        photo_hash = this.database_yml.getPhoto_hash().get("case_" + i);
      }
      ...
      Operation operation = Insert.into(NORMAL_TABLE_NAME)
                                  .columns("serial_num",
                                          ...
                                          "photo_hash")
                                  .values(i,
                                          this.database_yml.getFaci_id().get("case_" + i), 
                                          photo_name, 
                                          photo_hash)
                                  .build();
  ...
  public Facility_Photo compareEntityMake(Integer testcase_num, Boolean link_samba) throws IOException{
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      Facility_Photo mock_entity = mock(Facility_Photo.class);

      when(mock_entity.getSerial_num()).thenReturn(testcase_num);
      when(mock_entity.getFaci_id()).thenReturn(this.database_yml.getFaci_id().get("case_" + testcase_num));
      when(mock_entity.getPhoto_name()).thenReturn(this.database_yml.getPhoto_name().get("case_" + testcase_num));

      if(link_samba){
        when(mock_entity.getPhoto_hash()).thenReturn(makeHashString(testcase_num));
      }else{
        when(mock_entity.getPhoto_hash()).thenReturn(this.database_yml.getPhoto_hash().get("case_" + testcase_num));
      }
   ...
   * - ただし設備番号に関しては参照制約があり、適当な値が入れれないので、若干既存のテストケースと被る。
   * - 範囲外の数字が来たときは処理を続行できないのでエラーを投げる。
   * 
   * @param[in] testcase_num 指定するテストケースの番号
   * 
  ...
  public Facility_Photo UpdateAfterEntityMake(Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      when(mock_entity.getSerial_num()).thenReturn(testcase_num);
      when(mock_entity.getFaci_id()).thenReturn(this.database_yml.getFaci_id().get("case_1"));
      when(mock_entity.getPhoto_name()).thenReturn("Blank");
      when(mock_entity.getPhoto_hash()).thenReturn("Blank");
  ...
  public Facility_Photo justInTimeEntityMake(){
    ...
    when(mock_entity.getFaci_id()).thenReturn(this.database_yml.getFaci_id().get("ok_length"));
    when(mock_entity.getPhoto_name()).thenReturn(this.database_yml.getPhoto_name().get("ok_length"));
    when(mock_entity.getPhoto_hash()).thenReturn(this.database_yml.getPhoto_hash().get("ok_length"));
  ...
  public List<Facility_Photo> failedEntityMake() throws IOException{
    ...
    List<Facility_Photo> fail_mock_list = new ArrayList<>();
    int i = 1;

    String[] faci_id = {"ng_overflow", "ng_foreign"};
    for(String key_name: faci_id){
      Facility_Photo mock_entity = compareEntityMake(1, false);

      when(mock_entity.getSerial_num()).thenReturn(i++);
      when(mock_entity.getFaci_id()).thenReturn(this.database_yml.getFaci_id().get(key_name));

      fail_mock_list.add(mock_entity);
    }
    
    String[] photo_name = {"ng_overflow", "ng_null"};
    for(String key_name: photo_name){
      Facility_Photo mock_entity = compareEntityMake(1, false);
            
      when(mock_entity.getSerial_num()).thenReturn(i++);
      when(mock_entity.getPhoto_name()).thenReturn(this.database_yml.getPhoto_name().get(key_name));

      fail_mock_list.add(mock_entity);
    }

    String[] photo_hash = {"ng_overflow", "ng_null"};
    for(String key_name: photo_hash){
      Facility_Photo mock_entity = compareEntityMake(1, false);
            
      when(mock_entity.getSerial_num()).thenReturn(i++);
      when(mock_entity.getPhoto_hash()).thenReturn(this.database_yml.getPhoto_hash().get(key_name));
  ...
  public void historyDatabaseSetup(Boolean link_samba) throws IOException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      //Sambaとのリンクを指定されている場合は、データをSambaにも保存しハッシュ値を登録。
      if(link_samba){
        String filename = i + ".gz";
        photo_name = this.database_yml.getPhoto_data().get("case_" + i);
        photo_hash = this.samba_testcase.historySambaSetup(photo_name, filename);
      }else{
        photo_name = this.database_yml.getPhoto_name().get("case_" + i);
        photo_hash = this.database_yml.getPhoto_hash().get("case_" + i);
      }
  ...
  public Facility_Photo_History compareHistoryEntityMake(Integer testcase_num, Boolean link_samba) throws IOException{
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      Facility_Photo_History mock_entity = mock(Facility_Photo_History.class);
      ...
      when(mock_entity.getSerial_num()).thenReturn(testcase_num);
      when(mock_entity.getFaci_id()).thenReturn(this.hist_testcase_make.getCompareId(testcase_num));
      when(mock_entity.getPhoto_name()).thenReturn(this.database_yml.getPhoto_name().get("case_" + testcase_num));
  ...
  public List<Facility_Photo_History> failedHistoryEntityMake() throws IOException{
    ...
    List<Facility_Photo_History> fail_mock_list = new ArrayList<>();
    int i = 0;

    for(Date fail_data: this.hist_testcase_make.getFailChange_datetime()){
      Facility_Photo_History mock_entity = compareHistoryEntityMake(1, false);
  ...
  private String makeHashString(Integer testcase_num) throws IOException{
    ...
    String file_name = this.database_yml.getPhoto_data().get("case_" + testcase_num);
    String file_path = "src/main/resources/TestCaseFile/FacilityPhoto/BlobTestCaseData/setup/" + file_name;
  ...
  public String compareSambaFileMake(Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      return this.database_yml.getPhoto_data().get("case_" + testcase_num);
    }
  }
