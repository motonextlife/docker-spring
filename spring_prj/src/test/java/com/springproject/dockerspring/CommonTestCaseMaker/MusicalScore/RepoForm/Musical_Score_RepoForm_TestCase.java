 ...
 * 
 * @brief [楽譜管理]に関するテストケース生成処理のうち、[データベースのフリーワード検索]
 ...
 * @brief 主に[楽譜管理]機能のテストにおいて、リポジトリの検索のテストを行う際の検索
 ...
 * - @Component
 * 
 * @see Musical_Score_RepoForm_Yaml
 **************************************************************************************
 */ 
@Component
public class Musical_Score_RepoForm_TestCase{
  
  private final Musical_Score_RepoForm_Yaml repoform_yml;
  ...
  //! 楽譜番号
  public static final Integer SCORE_ID = 2;

  //! 購入日
  public static final Integer BUY_DATE = 3;
  ...
  //! 編曲者
  public static final Integer ARRANGER = 6;

  //! 出版社
  public static final Integer PUBLISHER = 7;

  //! 保管場所
  public static final Integer STORAGE_LOC = 8;

  //! 廃棄日
  public static final Integer DISP_DATE = 9;

  //! その他コメント
  public static final Integer COMMENT = 10;
   ...
   **********************************************************************************************
   */ 
  public Musical_Score_RepoForm_TestCase() throws IOException{

    try(InputStream in_repoform_yml = new ClassPathResource("TestCaseFile/MusicalScore/musical-score-repoform.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.repoform_yml = yaml.loadAs(in_repoform_yml, Musical_Score_RepoForm_Yaml.class);
    }
  }
  ...
  public String[] otherRepoForm(Musical_Score_TestKeys subject_key, Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      switch(subject_key){
        case Score_Id:
          return this.repoform_yml.getScore_id().get("word_" + testcase_num);
        case Buy_Date:
        ...
        case Arranger:
          return this.repoform_yml.getArranger().get("word_" + testcase_num);
        case Publisher:
