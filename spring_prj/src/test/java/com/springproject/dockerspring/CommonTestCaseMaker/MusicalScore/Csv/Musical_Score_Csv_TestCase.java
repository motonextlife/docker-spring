 ...
 * 
 * @brief [楽譜管理]に関するテストケース生成処理のうち、[CSVファイルの入出力]
...
/** 
 **************************************************************************************
 * @file Musical_Score_Csv_TestCase.java
 * @brief 主に[楽譜管理]機能のテストにおいて、CSVファイルに関係するテストケースを読み込む
 * クラスを格納するファイル。
 * 
 * @note
 ...
 */ 
@Component
public class Musical_Score_Csv_TestCase{
  
  private final Musical_Score_Csv_Yaml csv_yml;
   ...
   **********************************************************************************************
   */
  public Musical_Score_Csv_TestCase() throws IOException{

    try(InputStream in_csv_yml = new ClassPathResource("TestCaseFile/MusicalScore/musical-score-csv.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.csv_yml = yaml.loadAs(in_csv_yml, Musical_Score_Csv_Yaml.class);
    }
  }
