 ...
 * 
 * @brief [楽譜データ管理機能]に関するテストケース生成処理のうち、[データベースのフリーワード検索]
...
/** 
 **************************************************************************************
 * @file Score_Pdf_RepoForm_TestCase.java
 * @brief 主に[楽譜データ管理]機能のテストにおいて、リポジトリの検索のテストを行う際の検索
 ...
 * - @Component
 * 
 * @see Score_Pdf_RepoForm_Yaml
 **************************************************************************************
 */ 
@Component
public class Score_Pdf_RepoForm_TestCase{
  
  private final Score_Pdf_RepoForm_Yaml repoform_yml;
  ...
  //! 楽譜ファイル名
  public static final Integer SCORE_NAME = 2;
   ...
   **********************************************************************************************
   */ 
  public Score_Pdf_RepoForm_TestCase() throws IOException{

    try(InputStream in_repoform_yml = new ClassPathResource("TestCaseFile/ScorePdf/score-pdf-repoform.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.repoform_yml = yaml.loadAs(in_repoform_yml, Score_Pdf_RepoForm_Yaml.class);
    }
  }
  ...
  public String[] otherRepoForm(Score_Pdf_TestKeys subject_key, Integer testcase_num){
    ...
    if(testcase_num > 20 || testcase_num < 1){
      ...
      switch(subject_key){
        ...
        case Score_Name:
          return this.repoform_yml.getScore_name().get("word_" + testcase_num);
        default:
          throw new IllegalArgumentException();
      }      
    }
