 ...
 * 
 * @brief [音源データ管理機能]に関するテストケース生成処理のうち、[ZIP圧縮ファイルの入出力]
...
/** 
 **************************************************************************************
 * @file Audio_Data_Zip_TestCase.java
 * @brief 主に[音源データ機能]機能のテストにおいて、ZIPの入出力の機能において使用するテスト
 ...
 * - @Component
 * 
 * @see Audio_Data_Zip_Yaml
 **************************************************************************************
 */ 
@Component
public class Audio_Data_Zip_TestCase{
  
  private final Audio_Data_Zip_Yaml zip_yml;
   ...
   **********************************************************************************************
   */ 
  public Audio_Data_Zip_TestCase() throws IOException{

    try(InputStream in_zip_yml = new ClassPathResource("TestCaseFile/AudioData/audio-data-zip.yml").getInputStream()){
      Yaml yaml = new Yaml();
      this.zip_yml = yaml.loadAs(in_zip_yml, Audio_Data_Zip_Yaml.class);
    }
  }
  ...
  /** 
   **********************************************************************************************
   * @brief 項目[音源ZIPファイル]でバリデーションエラーになるテストケースを読み取って、リストを作成する。
   * 
   * @details
  ...
  public List<String> getZipFailedData(Boolean append_form){
    ...
    if(append_form){
      fail_list.add(this.zip_yml.getZip_file().get("ng_no_wav_mp3"));
      fail_list.add(this.zip_yml.getZip_file().get("ng_no_wav_aac"));
