 ...
 * 
 * @brief [楽譜データ管理機能]に関するテストケース生成処理のうち、[出力失敗ケースのデータベース]
...
/** 
 **************************************************************************************
 * @file Score_Pdf_Fail_Database_TestCase.java
 * @brief 主に[楽譜データ情報]機能のテストにおいて、データベースとの通信テストの際に、あらかじめ
...
import com.springproject.dockerspring.CommonTestCaseMaker.ScorePdf.Form.Score_Pdf_Form_Yaml;
 ...
 * @see Score_Pdf_Fail_Samba_TestCase
 **************************************************************************************
 */ 
@Component
@Import({ConfigureFactory.class, History_TestCase_Make.class})
public class Score_Pdf_Fail_Database_TestCase{
  
  private final Score_Pdf_Form_Yaml form_yml;
  private final History_TestCase_Make hist_testcase_make;
  private final Score_Pdf_Fail_Samba_TestCase samba_fail_testcase;
  ...
  private final String NORMAL_TABLE_NAME = "Score_Pdf";
  private final String HISTORY_TABLE_NAME = "Score_Pdf_History";
   ...
   * @param[in] samba_fail_testcase [楽譜データ情報]のファイルサーバー関連のエラーテストケースクラス
   * 
   * @par 処理の大まかな流れ
   ...
   */ 
  @Autowired
  public Score_Pdf_Fail_Database_TestCase(ConfigureFactory config_factory, 
                                          History_TestCase_Make hist_testcase_make, 
                                          Score_Pdf_Fail_Samba_TestCase samba_fail_testcase) throws IOException{
  ...
  public Map<Integer, String> failDatabaseSetup(Boolean link_samba) throws IOException{
    ...
    List<Map<Score_Pdf_TestKeys, String>> fail_list = getDatabaseFail(link_samba);
    Map<Integer, String> search_map = new HashMap<>();

    int i = 1;
    for(Map<Score_Pdf_TestKeys, String> fail_map: fail_list){
      String score_name = "";
      String pdf_hash = "";

      if(link_samba){
        String filename = i + ".wav";
        score_name = fail_map.get(Score_Pdf_TestKeys.Pdf_Data);
        pdf_hash = this.samba_fail_testcase.failSambaSetup(score_name, filename);
      }else{
        score_name = fail_map.get(Score_Pdf_TestKeys.Score_Name);
        pdf_hash = "EnterSuitableString";
      }
      ...
      Operation operation = Insert.into(NORMAL_TABLE_NAME)
                                  .columns("serial_num",
                                          ...
                                          "pdf_hash")
                                  .values(i,
                                          fail_map.get(Score_Pdf_TestKeys.Score_Id), 
                                          score_name, 
                                          pdf_hash)
                                  .build();

      operation_list.add(operation);
      search_map.put(i++, fail_map.get(Score_Pdf_TestKeys.Score_Id));
    }
  ...
  public List<Integer> historyFailDatabaseSetup(Boolean link_samba) throws IOException, ParseException{
    ...
    for(Map<String, String> fail_map: fail_list){
      ...
      if(link_samba){
        ...
        score_name = fail_map.get("pdf_data");
        pdf_hash = this.samba_fail_testcase.historyFailSambaSetup(score_name, filename);
      }else{
        score_name = fail_map.get("score_name");
        pdf_hash = "EnterSuitableString";
      }
   ...
   * -# 楽譜番号が参照元テーブルの楽譜番号を参照できるように、「楽譜番号がエラーのケース」以外のエラーケースに
   * 関しては、確実に参照できてエラーが起こらない楽譜番号に上書きする。
   * -# 取り出したデータは、順番にリストに格納する。
   * -# 各項目の格納が終わったリストを全て結合し、一つのリストにしたものを戻り値とする。
   **********************************************************************************************
   */ 
  private List<Map<Score_Pdf_TestKeys, String>> getDatabaseFail(Boolean link_samba){
    List<Map<Score_Pdf_TestKeys, String>> error_list = new ArrayList<>();

    if(link_samba){
      error_list.addAll(makePdfDataFailedData());
    }else{
      error_list.addAll(makeScoreNameFailedData());
    }

    for(Map<Score_Pdf_TestKeys, String> id_override: error_list){
      String score_id = id_override.get(Score_Pdf_TestKeys.Score_Id);
      score_id = score_id.replaceFirst("....$", "") + "_" + 1;

      id_override.put(Score_Pdf_TestKeys.Score_Id, score_id);
    }
  ...
  private List<Map<String, String>> getHistoryDatabaseFail(Boolean link_samba){
    ...
    List<Map<Score_Pdf_TestKeys, String>> normal_table = getDatabaseFail(link_samba);
    List<Map<History_TestKeys, String>> hist_table = this.hist_testcase_make.getHistoryDatabaseFailData();


    for(Map<Score_Pdf_TestKeys, String> normal_map: normal_table){
      ...
      strkey_map.put("score_id", normal_map.get(Score_Pdf_TestKeys.Score_Id));

      if(link_samba){
        strkey_map.put("pdf_data", normal_map.get(Score_Pdf_TestKeys.Pdf_Data));
      }else{
        strkey_map.put("score_name", normal_map.get(Score_Pdf_TestKeys.Score_Name));
      }
    ...
    for(Map<History_TestKeys, String> hist_map: hist_table){
      ...
      strkey_map.put("score_id", makeBaseData().get(Score_Pdf_TestKeys.Score_Id));

      if(link_samba){
        strkey_map.put("pdf_data", makeBaseData().get(Score_Pdf_TestKeys.Pdf_Data));
      }else{
        strkey_map.put("score_name", makeBaseData().get(Score_Pdf_TestKeys.Score_Name));
      }
   ...
   **********************************************************************************************
   */ 
  private Map<Score_Pdf_TestKeys, String> makeBaseData(){

    Map<Score_Pdf_TestKeys, String> base_map = new HashMap<>();
    String childkey = "ok";

    base_map.put(Score_Pdf_TestKeys.Score_Id, this.form_yml.getScore_id().get(childkey));
    base_map.put(Score_Pdf_TestKeys.Score_Name, this.form_yml.getScore_name().get(childkey));
    base_map.put(Score_Pdf_TestKeys.Pdf_Data, this.form_yml.getPdf_data().get(childkey));
   ...
   **********************************************************************************************
   */ 
  private List<Map<Score_Pdf_TestKeys, String>> makeScoreIdFailedData(){
    ...
    for(String key: key_name){
      Map<Score_Pdf_TestKeys, String> base_data = makeBaseData();
      base_data.put(Score_Pdf_TestKeys.Score_Id, this.form_yml.getScore_id().get(key));
  ...
  /** 
   **********************************************************************************************
   * @brief 項目[楽譜データ名]でエラーになるテストケースを格納したマップリストを作成する。
   * 
   * @details
   * - マッピングしたエンティティから該当項目のエラーケースを取り出し、ベースとなる合格ケースのリストを
   * 上書きすることで作成する。
   ...
   **********************************************************************************************
   */ 
  private List<Map<Score_Pdf_TestKeys, String>> makeScoreNameFailedData(){
    ...
    for(String key: key_name){
      Map<Score_Pdf_TestKeys, String> base_data = makeBaseData();
      base_data.put(Score_Pdf_TestKeys.Score_Name, this.form_yml.getScore_name().get(key));
      error_list.add(base_data);
    }
  ...
  /** 
   **********************************************************************************************
   * @brief 項目[楽譜データ]でエラーになるテストケースを格納したマップリストを作成する。
   * 
   * @details
   * - マッピングしたエンティティから該当項目のエラーケースを取り出し、ベースとなる合格ケースのリストを
   * 上書きすることで作成する。
   ...
   **********************************************************************************************
   */ 
  private List<Map<Score_Pdf_TestKeys, String>> makePdfDataFailedData(){

    List<Map<Score_Pdf_TestKeys, String>> error_list = new ArrayList<>();

    String[] key_name = {"ng_no_pdf_mid", 
                         "ng_no_pdf_svg", 
                         "ng_overflow", 
    ...
    for(String key: key_name){
      Map<Score_Pdf_TestKeys, String> base_data = makeBaseData();
      base_data.put(Score_Pdf_TestKeys.Pdf_Data, this.form_yml.getPdf_data().get(key));
      error_list.add(base_data);
    }
