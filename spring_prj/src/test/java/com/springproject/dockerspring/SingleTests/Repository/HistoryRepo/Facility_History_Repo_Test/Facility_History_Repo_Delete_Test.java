 ...
 * 
 * @brief 履歴用リポジトリのうち、[設備情報変更履歴]に関するテストを格納するパッケージ
 * 
 * @details
...
/** 
 **************************************************************************************
 * @file Facility_History_Repo_Delete_Test.java
 * @brief 主に[設備情報変更履歴]のリポジトリクラスで、データの削除を行うメソッドのテストを行う
 * クラスを格納したファイル。
 * 
 * @note
...
/** 
 **************************************************************************************
 * @brief 主に[設備情報変更履歴]のリポジトリクラスで、データの削除を行うメソッドのテストを
 * 行うクラス
 * 
 * @details 
...
public class Facility_History_Repo_Delete_Test{
  ...
  public void testcaseSetup(Facility_TestCase_Make testcase, 
                            Facility_History_Repo test_repo){

    Facility_History_Repo_Delete_Test.testcase = testcase;
    Facility_History_Repo_Delete_Test.test_repo = test_repo;
    ...
    Facility_History_Repo_Delete_Test.compare_entity = compare_entity;
  }
  ...
  public class Delete_Success {
    ...
    public Delete_Success() throws IOException{
      this.test_repo = Facility_History_Repo_Delete_Test.test_repo;
      this.compare_entity = Facility_History_Repo_Delete_Test.compare_entity;
      this.softly = Facility_History_Repo_Delete_Test.softly;

      Facility_History_Repo_Delete_Test.testcase.historyDatabaseSetup();
    }
    ...
    public void 削除の成功ケース(){
      ...
      for(int i = 0; i < 10; i++){
        Facility_History del_ent = this.compare_entity.get(i);
      ...
      for(int i = 10; i < 20; i++){
        Facility_History no_delete = this.compare_entity.get(i);

        Facility_History result = this.test_repo.findById(no_delete.getSerial_num()).get();
    ...
    public static void testcaseReset() throws IOException{
      Facility_History_Repo_Delete_Test.testcase.historyDatabaseReset();
    }
  }
  ...
  public class Period_Delete_Success {
    ...
    public void 該当期間データの削除の成功ケース() throws ParseException, InterruptedException{
      ...
      String[] assume = Facility_History_Repo_Delete_Test.testcase
      ...
      List<Facility_History> compare = Common_Repository_TestUtils
        .compareListMake(CompareParam
                         ...
                         .sort_first(Comparator.comparing(Facility_History::getHistory_id))
                         .skip(0)
      ...
      List<Facility_History> get_data = (List<Facility_History>) this.test_repo.findAll();
      List<Facility_History> result = get_data.stream()
                                              .sorted(comparing(Facility_History::getHistory_id))
      ...
      for(int i = 0; i < compare_size; i++){
        Facility_Quad_Repository_TestUtils.assertAllEquals(compare.get(i), result.get(i), this.softly, true);
      }
