/** 
 **************************************************************************************
 * @file Facility_Photo_SambaImpl_FileInput_Test.java
 * @brief 主に[設備写真データ管理]のファイルサーバーのデータ入力機能のテストを行うクラスを格納した
 * ファイル。
 **************************************************************************************
 */ 
...
/** 
 **************************************************************************************
 * @brief 主に[設備写真データ管理]のファイルサーバーのデータ入力機能のテストを行うクラス
 * 
 * @details 
...
public class Facility_Photo_SambaImpl_FileInput_Test implements SambaProcess_FileInput_Test{
  ...
  private final String prefix = "FacilityPhotoSambaImplFileInputTest";
  ...
  private void configStaticSet(){
    Facility_Photo_SambaImpl_FileInput_Test.config_static = this.config;
  }
  ...
  private void fileCheckAndReset(File compare_file, String file_name, String ext, Boolean hist) 
    ...
    String file_path = Facility_Double_Component_TestUtils.sambaMakePath(file_name, ext, hist, this.config);

    try(BufferedInputStream bis_1 = new BufferedInputStream(new FileInputStream(compare_file));
        SmbFile smb = new SmbFile(file_path, Facility_Double_Component_TestUtils.sambaTestSetup(this.config));
      ...
      Facility_Double_Component_TestUtils.deleteTmpFileCheck(compare_file, this.softly);
      Facility_Double_Component_TestUtils.deleteSambaFileCheck(file_name, ext, hist, this.config, this.softly);
    }
  }
  ...
  private File preparaSourceFile(String file_kind) throws IOException{
    ...
    String update_data = this.testcase.getPhotoDataFailedData().get(0);
    String other_data = this.testcase.getPhotoDataFailedData().get(1);
    String draft_data = this.testcase.getPhotoDataFailedData().get(2);

    switch(file_kind){
      case "source":
        return Facility_Double_Component_TestUtils.
  ...
  public void 共通クラス側のファイルサーバーへのデータ入力機能の動作確認() throws IOException{
    ...
    String draft_path = Facility_Double_Component_TestUtils.sambaMakePath("1", this.ext, false, this.config);
    String other_path = Facility_Double_Component_TestUtils.sambaMakePath("2", this.ext, false, this.config);
    Facility_Double_Component_TestUtils.preparaSambaData(draft_source, draft_path, this.config, false);
    Facility_Double_Component_TestUtils.preparaSambaData(other_source, other_path, this.config, false);
  ...
  public void 集約クラス側のファイルサーバーへの通常データ入力機能の動作確認() throws IOException{
    ...
    this.faci_samba.fileInsertUpdate(normal_file, 1);
    fileCheckAndReset(source, "1", this.ext, false);
    ...
    this.faci_samba.fileInsertUpdate(update, 1);
  ...
  public void 集約クラス側のファイルサーバーへの履歴データ入力機能の動作確認() throws IOException{
    ...
    this.faci_samba.historyFileInsert(normal_file, 1);
  ...
  public static void ファイルサーバーと一時ファイルディレクトリの初期化確認() throws InterruptedException, IOException{
    ...
    Facility_Double_Component_TestUtils.checkSambaClear(Facility_Photo_SambaImpl_FileInput_Test.config_static);
  }
}
