/** 
 **************************************************************************************
 * @file Score_Pdf_SambaImpl_FileDelete_Test.java
 * @brief 主に[楽譜データ管理]のファイルサーバーのデータ削除機能のテストを行うクラスを格納した
 * ファイル。
 **************************************************************************************
 */ 
...
/** 
 **************************************************************************************
 * @brief 主に[楽譜データ管理]のファイルサーバーのデータ削除機能のテストを行うクラス
 * 
 * @details 
...
public class Score_Pdf_SambaImpl_FileDelete_Test implements SambaProcess_FileDelete_Test{
  ...
  private final String prefix = "ScorePdfSambaImplFileDeleteTest";
  ...
  private void configStaticSet(){
    Score_Pdf_SambaImpl_FileDelete_Test.config_static = this.config;
  }
  ...
  private void fileDeleteCheck(String delete_filename, String remain_filename, String ext, Boolean hist)
    ...
    String delete_filename_path = Score_Double_Component_TestUtils.sambaMakePath(delete_filename, ext, hist, this.config);
    String remain_filename_path = Score_Double_Component_TestUtils.sambaMakePath(remain_filename, ext, hist, this.config);

    try(SmbFile smb = new SmbFile(delete_filename_path, Score_Double_Component_TestUtils.sambaTestSetup(this.config))) {
      this.softly.assertThat(smb.exists()).isFalse();
    }

    try(SmbFile smb = new SmbFile(remain_filename_path, Score_Double_Component_TestUtils.sambaTestSetup(this.config))) {
  ...
  public void 共通クラス側のファイルサーバー内のデータ削除機能の動作確認() throws IOException{
    ...
    File normal_file = Score_Double_Component_TestUtils.
    ...
    Score_Double_Component_TestUtils.preparaSambaData(normal_file, normal_path_1, this.config, false);
    Score_Double_Component_TestUtils.preparaSambaData(normal_file, normal_path_2, this.config, false);
    Score_Double_Component_TestUtils.preparaSambaData(hist_file, history_path_1, this.config, false);
    Score_Double_Component_TestUtils.preparaSambaData(hist_file, history_path_2, this.config, false);
    ...
    Score_Double_Component_TestUtils.deleteTmpFileCheck(hist_file, this.softly);
    Score_Double_Component_TestUtils.deleteSambaFileCheck("2", this.ext, false, this.config, this.softly);
    Score_Double_Component_TestUtils.deleteSambaFileCheck("2", this.hist_ext, true, this.config, this.softly);
  ...
  public void 集約クラス側のファイルサーバー内の通常データ削除機能の動作確認() throws IOException{
    ...
    this.score_samba.fileDelete(1);

    fileDeleteCheck("1", "2", this.ext, false);

    this.softly.assertThatThrownBy(() -> this.score_samba.fileDelete(3))
               .isInstanceOf(IOException.class);
  ...
  public void 集約クラス側のファイルサーバー内の履歴データ削除機能の動作確認() throws IOException{
    ...
    this.score_samba.historyFileDelete(1);

    fileDeleteCheck("1", "2", this.hist_ext, true);

    this.softly.assertThatThrownBy(() -> this.score_samba.historyFileDelete(3))
               .isInstanceOf(IOException.class);
  ...
  public static void ファイルサーバーと一時ファイルディレクトリの初期化確認() throws InterruptedException, IOException{
    ...
    Score_Double_Component_TestUtils.checkSambaClear(Score_Pdf_SambaImpl_FileDelete_Test.config_static);
  }
}
