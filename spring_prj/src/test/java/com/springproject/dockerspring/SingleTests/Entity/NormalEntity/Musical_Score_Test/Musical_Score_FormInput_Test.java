...
public class Musical_Score_FormInput_Test implements Entity_FormInput_Test{
  ...
  private void setInitTargetForm(String str){
    ...
    Musical_Score_Form mock_form = mock(Musical_Score_Form.class);
    when(mock_form.getSerial_num()).thenReturn(null);
    when(mock_form.getScore_id()).thenReturn(str);
    when(mock_form.getSong_title()).thenReturn(str);
    when(mock_form.getComposer()).thenReturn(str);
    when(mock_form.getArranger()).thenReturn(str);
    ...
    assertAllNull(new Musical_Score(mock_form));
  }
  ...
  public void 入力したフォームクラスデータの同一性確認() throws ParseException{
    ...
    when(mock_form.getScore_id()).thenReturn(score_id);
    ...
    when(mock_form.getArranger()).thenReturn(arranger);
    ...
    Musical_Score entity = new Musical_Score(mock_form);
