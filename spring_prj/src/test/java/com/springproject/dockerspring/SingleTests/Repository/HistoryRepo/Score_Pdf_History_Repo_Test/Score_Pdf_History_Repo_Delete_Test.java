 ...
 * 
 * @brief 履歴用リポジトリのうち、[楽譜データ情報変更履歴]に関するテストを格納するパッケージ
 * 
 * @details
...
/** 
 **************************************************************************************
 * @file Score_Pdf_History_Repo_Delete_Test.java
 * @brief 主に[楽譜データ変更履歴]のリポジトリクラスで、データの削除を行うメソッドのテストを行う
 * クラスを格納したファイル。
 * 
 * @note
...
/** 
 **************************************************************************************
 * @brief 主に[楽譜データ変更履歴]のリポジトリクラスで、データの削除を行うメソッドのテストを
 * 行うクラス
 * 
 * @details 
...
public class Score_Pdf_History_Repo_Delete_Test{
  ...
  public void testcaseSetup(Score_Pdf_TestCase_Make testcase, 
                            Score_Pdf_History_Repo test_repo) throws IOException{

    Score_Pdf_History_Repo_Delete_Test.testcase = testcase;
    Score_Pdf_History_Repo_Delete_Test.test_repo = test_repo;
    ...
    Score_Pdf_History_Repo_Delete_Test.compare_entity = compare_entity;
  }
  ...
  public class Delete_Success {
    ...
    public Delete_Success() throws IOException{
      this.test_repo = Score_Pdf_History_Repo_Delete_Test.test_repo;
      this.compare_entity = Score_Pdf_History_Repo_Delete_Test.compare_entity;
      this.softly = Score_Pdf_History_Repo_Delete_Test.softly;

      Score_Pdf_History_Repo_Delete_Test.testcase.historyDatabaseSetup(false);
    }
    ...
    public void 削除の成功ケース(){
      ...
      for(int i = 0; i < 10; i++){
        Score_Pdf_History del_ent = this.compare_entity.get(i);
      ...
      for(int i = 10; i < 20; i++){
        Score_Pdf_History no_delete = this.compare_entity.get(i);

        Score_Pdf_History result = this.test_repo.findById(no_delete.getSerial_num()).get();
    ...
    public static void testcaseReset() throws IOException{
      Score_Pdf_History_Repo_Delete_Test.testcase.historyDatabaseReset(false);
    }
  }
}
