/** 
 **************************************************************************************
 * @file Sound_Source_CsvImpl_ExtractCsv_Test.java
 * @brief 主に[音源管理]のCSVデータのデータ抽出機能のテストを行うクラスを格納した
 * ファイル。
 * 
...
/** 
 **************************************************************************************
 * @brief 主に[音源管理]のCSVデータのデータ抽出機能のテストを行うクラス
 * 
 * @details 
...
public class Sound_Source_CsvImpl_ExtractCsv_Test implements CsvProcess_ExtractCsv_Test{
  ...
  private final String prefix = "SoundSourceCsvImplExtractCsvTest";
   ...
   */
  @Autowired
  public Sound_Source_CsvImpl_ExtractCsv_Test(Sound_Source_TestCase_Make testcase){
  ...
  public void 共通クラス側のCSV入力データ抽出機能確認の抽出成功パターン() throws IOException, InputFileDifferException{
    ...
    File ok_file = Sound_Double_Component_TestUtils.generateTmpFile(this.target_func, 
                                                                    this.target_dir, 
  ...
  public void 集約クラス側のCSV入力データ抽出機能確認の抽出成功パターン() throws IOException, InputFileDifferException{
    ...
    try(FileInputStream fis = new FileInputStream(ok_file);
      ...
      MockMultipartFile mock_file = new MockMultipartFile("no_name", bis);
      tmp_file_path = this.sound_csv.extractCsv(mock_file);
    }
  ...
  public void 共通クラス側のCSV入力データ抽出機能確認の抽出失敗パターン() throws IOException, InputFileDifferException{
    ...
    for(String filename: fail_list){
      File fail_file = Sound_Double_Component_TestUtils.generateTmpFile(this.target_func, 
                                                                        this.target_dir, 
                                                                        filename, 
                                                                        this.prefix, 
                                                                        this.extension, 
  ...
  public void 集約クラス側のCSV入力データ抽出機能確認の抽出失敗パターン() throws IOException, InputFileDifferException{
    ...
    for(String filename: fail_list){
      ...
      try(BufferedInputStream bis = new BufferedInputStream(new FileInputStream(fail_file))){
        ...
        MockMultipartFile mock_file = new MockMultipartFile("no_name", bis);
        this.softly.assertThatThrownBy(() -> this.sound_csv.extractCsv(mock_file))
                   .isInstanceOf(InputFileDifferException.class);
      }
    }

    this.softly.assertThatThrownBy(() -> this.sound_csv.extractCsv(null))
               .isInstanceOf(NullPointerException.class);
