/** 
 **************************************************************************************
 * @file Score_Pdf_History_Repo_SearchCount_Test.java
 * @brief 主に[楽譜データ変更履歴]のリポジトリクラスで、独自に追加した検索結果を取得するメソッドと
 ...
 **************************************************************************************
 */ 
@Import({Score_Pdf_TestCase_Make.class, RepoConfig.class})
@EnableAsync
@DataJdbcTest
@AutoConfigureTestDatabase(replace = Replace.NONE)
public class Score_Pdf_History_Repo_SearchCount_Test{
  ...
  private final List<Score_Pdf_History> origin_compare;
   ...
   */
  @Autowired
  public Score_Pdf_History_Repo_SearchCount_Test(Score_Pdf_History_Repo test_repo, 
                                                 Score_Pdf_TestCase_Make testcase) throws IOException{
  ...
  private void configStaticSet(){
    Score_Pdf_History_Repo_SearchCount_Test.testcase_static = this.testcase;
  }
   ...
   **********************************************************************************************
   */
  private void findAllSearchCheck(Function<TestParam<Score_Pdf_History>, CompletableFuture<List<Score_Pdf_History>>> find_all_method, 
                                  TestParamBuilder<Score_Pdf_History> test_builder) 
                                  throws ExecutionException, InterruptedException{

    BiConsumer<Score_Pdf_History, Score_Pdf_History> assert_method = (compare, result) -> {
      Score_Quad_Repository_TestUtils.assertAllEquals(compare, result, this.softly, true);
    };

    TestParam<Score_Pdf_History> test_param = test_builder.assert_method(assert_method)
                                                          .softly(this.softly)
                                                          .build();
  ...
  public void 検索対象のカラムが履歴日時の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      CompareParam<Score_Pdf_History> list_builder = CompareParam
                                      .<Score_Pdf_History>builder()
      ...
      List<Score_Pdf_History> compare_1 = Common_Repository_TestUtils
                              .compareListMake(list_builder.toBuilder()
                                                           .sort_first(Comparator.comparing(Score_Pdf_History::getChange_datetime))
                                                           .sort_second(Comparator.comparing(Score_Pdf_History::getHistory_id))
                                                           .build());
      List<Score_Pdf_History> compare_2 = Common_Repository_TestUtils
                              .compareListMake(list_builder.toBuilder()
                                                           .sort_first(Comparator.comparing(Score_Pdf_History::getChange_datetime, 
      ...
      Function<TestParam<Score_Pdf_History>, CompletableFuture<List<Score_Pdf_History>>> find_all_method_1 = s -> {
        return this.test_repo.findAllByDateBetween(s.getStart_datetime(), s.getEnd_datetime(), s.getLimit(), s.getOffset());
      };

      Function<TestParam<Score_Pdf_History>, CompletableFuture<List<Score_Pdf_History>>> find_all_method_2 = s -> {
        return this.test_repo.findAllByDateBetweenDESC(s.getStart_datetime(), s.getEnd_datetime(), s.getLimit(), s.getOffset());
      };

      TestParam<Score_Pdf_History> test_builder = TestParam.<Score_Pdf_History>builder().start_datetime(start)
                                                                                        .end_datetime(end)
                                                                                        .limit(limit)
                                                                                        .offset(offset)
                                                                                        .all_judge(true)
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findByScore_id][findByScore_idDESC]を実行した際の
   * 検索結果を判定する。
   *
   * @details
   * - テストの内容としては、検索ワードを渡してメソッドを実行した際に、想定された検索結果と、テスト対象メソッド
   ...
   */
  @Test
  public void 検索対象のカラムが楽譜番号の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      List<Score_Pdf_History> compare_1 = Common_Repository_TestUtils
                              .compareListMake(list_builder.toBuilder()
                                                           .sort_first(Comparator.comparing(Score_Pdf_History::getScore_id))
                                                           .sort_second(Comparator.comparing(Score_Pdf_History::getChange_datetime))
                                                           .sort_third(Comparator.comparing(Score_Pdf_History::getHistory_id))
                                                           .build());
      List<Score_Pdf_History> compare_2 = Common_Repository_TestUtils
                              .compareListMake(list_builder.toBuilder()
                                                           .sort_first(Comparator.comparing(Score_Pdf_History::getScore_id, 
      ...
      Function<TestParam<Score_Pdf_History>, CompletableFuture<List<Score_Pdf_History>>> find_all_method_1 = s -> {
        return this.test_repo.findByScore_id(s.getMain_word(), s.getStart_datetime(), s.getEnd_datetime(), 
                                             s.getLimit(), s.getOffset());
      };

      Function<TestParam<Score_Pdf_History>, CompletableFuture<List<Score_Pdf_History>>> find_all_method_2 = s -> {
        return this.test_repo.findByScore_idDESC(s.getMain_word(), s.getStart_datetime(), s.getEnd_datetime(), 
                                                 s.getLimit(), s.getOffset());
      };

      TestParam<Score_Pdf_History> test_builder = TestParam.<Score_Pdf_History>builder().main_word(word)
  ...
  public void 検索対象のカラムが操作ユーザー名の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      List<Score_Pdf_History> compare_1 = Common_Repository_TestUtils
                              .compareListMake(list_builder.toBuilder()
                                                           .sort_first(Comparator.comparing(Score_Pdf_History::getOperation_user))
                                                           .sort_second(Comparator.comparing(Score_Pdf_History::getChange_datetime))
                                                           .sort_third(Comparator.comparing(Score_Pdf_History::getHistory_id))
                                                           .build());
      List<Score_Pdf_History> compare_2 = Common_Repository_TestUtils
   ...
   */
  @Test
  public void カウント対象のカラムが楽譜番号の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      CompletableFuture<Integer> result_1 = this.test_repo.findByScore_idCount(word, start, end, limit);
      CompletableFuture<Integer> result_2 = this.test_repo.findByScore_idCount(word, start, null, limit);
      CompletableFuture<Integer> result_3 = this.test_repo.findByScore_idCount(word, null, end, limit);
      CompletableFuture<Integer> result_4 = this.test_repo.findByScore_idCount(null, start, end, limit);
      CompletableFuture<Integer> result_5 = this.test_repo.findByScore_idCount(word, start, end, null);
  ...
  public static void testcaseReset() throws IOException{
    Score_Pdf_History_Repo_SearchCount_Test.testcase_static.historyDatabaseReset(false);
  }
}
