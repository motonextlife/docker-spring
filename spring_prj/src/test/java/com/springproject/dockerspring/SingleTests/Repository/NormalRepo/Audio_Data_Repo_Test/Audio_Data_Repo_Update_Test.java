/** 
 **************************************************************************************
 * @file Audio_Data_Repo_Update_Test.java
 * @brief 主に[音源データ管理]のリポジトリクラスで、データの更新を行うメソッドのテストを行う
 * クラスを格納したファイル。
 **************************************************************************************
 */ 
...
/** 
 **************************************************************************************
 * @brief 主に[音源データ管理]のリポジトリクラスで、データの更新を行うメソッドのテストを
 * 行うクラス
 * 
 * @details 
...
public class Audio_Data_Repo_Update_Test{
  ...
  public static List<Audio_Data> after_entity;
  ...
  public void testcaseSetup(Audio_Data_TestCase_Make testcase, 
    ...
    Audio_Data_Repo_Update_Test.testcase = testcase;
    Audio_Data_Repo_Update_Test.foreign_case = foreign_case;
    Audio_Data_Repo_Update_Test.test_repo = test_repo;
    Audio_Data_Repo_Update_Test.foreign_repo = foreign_repo;

    List<Audio_Data> compare_entity = new ArrayList<>();
    List<Audio_Data> after_entity = new ArrayList<>();
    for(int i = 1; i <= 20; i++){
    ...
    Audio_Data_Repo_Update_Test.compare_entity = compare_entity;
    Audio_Data_Repo_Update_Test.after_entity = after_entity;
    Audio_Data_Repo_Update_Test.just_entity = Sound_Quad_Repository_TestUtils
                                              .MockToReal(testcase.justInTimeEntityMake());

    List<Audio_Data> failed_ent_tmp = testcase.failedEntityMake();
    Audio_Data_Repo_Update_Test.failed_entity = new ArrayList<>();

    for(Audio_Data fail_ent: failed_ent_tmp){
      Audio_Data_Repo_Update_Test.failed_entity.add(Sound_Quad_Repository_TestUtils.MockToReal(fail_ent));
    }
  }
  ...
  public class Update_Success {
    ...
    private final List<Audio_Data> after_entity;
    ...
    public Update_Success() throws IOException{
      this.test_repo = Audio_Data_Repo_Update_Test.test_repo;
      this.compare_entity = Audio_Data_Repo_Update_Test.compare_entity;
      this.after_entity = Audio_Data_Repo_Update_Test.after_entity;
      this.just_entity = Audio_Data_Repo_Update_Test.just_entity;
      this.softly = Audio_Data_Repo_Update_Test.softly;

      Audio_Data_Repo_Update_Test.foreign_case.databaseSetup();
      ...
      Audio_Data_Repo_Update_Test.foreign_repo.save(foreign_just_entity);

      Audio_Data_Repo_Update_Test.testcase.databaseSetup(false);
    }
    ...
    public void 更新の成功ケース(){
      ...
      for(int i = 0; i < 10; i++){
        Audio_Data save_ent = this.after_entity.get(i);
        this.test_repo.save(save_ent);

        Audio_Data result = this.test_repo.findById(save_ent.getSerial_num()).get();
        Sound_Quad_Repository_TestUtils.assertAllEquals(save_ent, result, this.softly, true);
      }

      for(int i = 10; i < 20; i++){
        Audio_Data no_change = this.compare_entity.get(i);

        Audio_Data result = this.test_repo.findById(no_change.getSerial_num()).get();
      ...
      Audio_Data after = this.test_repo.findById(just_ent_serial).get();
    ...
    public static void testcaseReset() throws IOException{
      Audio_Data_Repo_Update_Test.testcase.databaseReset(false);
      Audio_Data_Repo_Update_Test.foreign_case.databaseReset();
    }
  }
  ...
  public class Update_Failed {
    ...
    public Update_Failed() throws IOException{
      this.test_repo = Audio_Data_Repo_Update_Test.test_repo;
      this.failed_entity = Audio_Data_Repo_Update_Test.failed_entity;
      this.softly = Audio_Data_Repo_Update_Test.softly;
  ...
  public class Foreign_Success {
    ...
    private final Audio_Data_Repo test_repo;
    private final Sound_Source_Repo foreign_repo;
    private final Audio_Data before_entity;
    ...
    public Foreign_Success() throws IOException{
      this.test_repo = Audio_Data_Repo_Update_Test.test_repo;
      this.foreign_repo = Audio_Data_Repo_Update_Test.foreign_repo;
      this.softly = Audio_Data_Repo_Update_Test.softly;
      
      this.before_entity = Audio_Data_Repo_Update_Test.compare_entity.get(0);
    ...
    public void 更新の参照元への追従確認() throws InterruptedException, ExecutionException{
      ...
      String target_id = this.before_entity.getSound_id();
      List<Audio_Data> target = this.test_repo.findBySound_id(target_id, 1, 0).get();

      String after_id = "change-after-001";
      Sound_Source foreign_entity = this.foreign_repo.findBySound_Id(target_id).get();
      foreign_entity.setSound_id(after_id);

      this.foreign_repo.save(foreign_entity);


      for(Audio_Data ent: target){
        Audio_Data after = this.test_repo.findById(ent.getSerial_num()).get();

        this.softly.assertThat(after.getSound_id()).isEqualTo(after_id);
      }
