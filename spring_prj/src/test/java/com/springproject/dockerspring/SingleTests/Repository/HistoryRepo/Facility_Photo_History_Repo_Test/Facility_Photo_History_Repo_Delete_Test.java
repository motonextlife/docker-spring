 ...
 * 
 * @brief 履歴用リポジトリのうち、[設備写真データ変更履歴]に関するテストを格納するパッケージ
 * 
 * @details
 * - このパッケージは、設備写真データ変更履歴のテストを行うクラスを格納する。
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.Repository.HistoryRepo.Facility_Photo_History_Repo_Test;
...
/** 
 **************************************************************************************
 * @file Facility_Photo_History_Repo_Delete_Test.java
 * @brief 主に[設備写真データ変更履歴]のリポジトリクラスで、データの削除を行うメソッドのテストを行う
 * クラスを格納したファイル。
 * 
 * @note
...
/** 
 **************************************************************************************
 * @brief 主に[設備写真データ変更履歴]のリポジトリクラスで、データの削除を行うメソッドのテストを
 * 行うクラス
 * 
 * @details 
...
public class Facility_Photo_History_Repo_Delete_Test{
  ...
  public void testcaseSetup(Facility_Photo_TestCase_Make testcase, 
                            Facility_Photo_History_Repo test_repo) throws IOException{

    Facility_Photo_History_Repo_Delete_Test.testcase = testcase;
    Facility_Photo_History_Repo_Delete_Test.test_repo = test_repo;
    ...
    Facility_Photo_History_Repo_Delete_Test.compare_entity = compare_entity;
  }
  ...
  public class Delete_Success {
    ...
    public Delete_Success() throws IOException{
      this.test_repo = Facility_Photo_History_Repo_Delete_Test.test_repo;
      this.compare_entity = Facility_Photo_History_Repo_Delete_Test.compare_entity;
      this.softly = Facility_Photo_History_Repo_Delete_Test.softly;

      Facility_Photo_History_Repo_Delete_Test.testcase.historyDatabaseSetup(false);
    }
    ...
    public void 削除の成功ケース(){
      ...
      for(int i = 0; i < 10; i++){
        Facility_Photo_History del_ent = this.compare_entity.get(i);
      ...
      for(int i = 10; i < 20; i++){
        Facility_Photo_History no_delete = this.compare_entity.get(i);

        Facility_Photo_History result = this.test_repo.findById(no_delete.getSerial_num()).get();
    ...
    public static void testcaseReset() throws IOException{
      Facility_Photo_History_Repo_Delete_Test.testcase.historyDatabaseReset(false);
    }
  }
}
