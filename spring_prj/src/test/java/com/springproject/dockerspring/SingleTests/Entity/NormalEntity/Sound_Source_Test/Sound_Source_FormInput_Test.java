...
public class Sound_Source_FormInput_Test implements Entity_FormInput_Test{
  ...
  private void setInitTargetForm(String str){
    ...
    Sound_Source_Form mock_form = mock(Sound_Source_Form.class);
    when(mock_form.getSerial_num()).thenReturn(null);
    when(mock_form.getSound_id()).thenReturn(str);
    when(mock_form.getSong_title()).thenReturn(str);
    when(mock_form.getComposer()).thenReturn(str);
    when(mock_form.getPerformer()).thenReturn(str);
    when(mock_form.getPublisher()).thenReturn(str);
    when(mock_form.getOther_comment()).thenReturn(str);

    assertAllNull(new Sound_Source(mock_form));
  }
  ...
  public void 入力したフォームクラスデータの同一性確認() throws ParseException{
    ...
    when(mock_form.getSound_id()).thenReturn(sound_id);
    when(mock_form.getUpload_date()).thenReturn(upload_date);
    when(mock_form.getSong_title()).thenReturn(song_title);
    when(mock_form.getComposer()).thenReturn(composer);
    when(mock_form.getPerformer()).thenReturn(performer);
    when(mock_form.getPublisher()).thenReturn(publisher);
    when(mock_form.getOther_comment()).thenReturn(other_comment);

    Sound_Source entity = new Sound_Source(mock_form);
