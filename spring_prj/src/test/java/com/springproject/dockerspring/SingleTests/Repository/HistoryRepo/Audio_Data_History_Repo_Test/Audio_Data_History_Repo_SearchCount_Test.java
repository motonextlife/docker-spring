/** 
 **************************************************************************************
 * @file Audio_Data_History_Repo_SearchCount_Test.java
 * @brief 主に[音源データ変更履歴]のリポジトリクラスで、独自に追加した検索結果を取得するメソッドと
...
public class Audio_Data_History_Repo_SearchCount_Test{
   ...
   */
  @Autowired
  public Audio_Data_History_Repo_SearchCount_Test(Audio_Data_History_Repo test_repo, 
  ...
  private void configStaticSet(){
    Audio_Data_History_Repo_SearchCount_Test.testcase_static = this.testcase;
  }
   ...
   **********************************************************************************************
   */
  private void findAllSearchCheck(Function<TestParam<Audio_Data_History>, CompletableFuture<List<Audio_Data_History>>> find_all_method, 
                                  TestParamBuilder<Audio_Data_History> test_builder) 
  ...
  public void 検索対象のカラムが履歴日時の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      Function<TestParam<Audio_Data_History>, CompletableFuture<List<Audio_Data_History>>> find_all_method_1 = s -> {
        return this.test_repo.findAllByDateBetween(s.getStart_datetime(), s.getEnd_datetime(), s.getLimit(), s.getOffset());
      };

      Function<TestParam<Audio_Data_History>, CompletableFuture<List<Audio_Data_History>>> find_all_method_2 = s -> {
        return this.test_repo.findAllByDateBetweenDESC(s.getStart_datetime(), s.getEnd_datetime(), s.getLimit(), s.getOffset());
      };

      TestParam<Audio_Data_History> test_builder = TestParam.<Audio_Data_History>builder().start_datetime(start)
                                                                                          .end_datetime(end)
                                                                                          .limit(limit)
                                                                                          .offset(offset)
                                                                                          .all_judge(true)
  ...
  public void 検索対象のカラムが音源番号の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      TestParam<Audio_Data_History> test_builder = TestParam.<Audio_Data_History>builder().main_word(word)
  ...
  public static void testcaseReset() throws IOException{
    Audio_Data_History_Repo_SearchCount_Test.testcase_static.historyDatabaseReset(false);
  }
}
