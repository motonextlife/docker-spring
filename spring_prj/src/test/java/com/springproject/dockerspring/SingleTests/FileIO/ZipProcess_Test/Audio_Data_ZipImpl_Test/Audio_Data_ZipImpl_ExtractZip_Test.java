/** 
 **************************************************************************************
 * @package com.springproject.dockerspring.SingleTests.FileIO.ZipProcess_Test
 * 
 * @brief ファイル入出力の単体テストのうち、[ZIPファイルの入出力]の機能に関するテストを行う
 * クラスを格納するパッケージ
 * 
 * @details
 * - このパッケージは、テストケースとして用意したZIPファイルの抽出機能確認や、保存データの
 * ZIPデータとしての出力機能をテストするクラスを格納している。
 **************************************************************************************
 */ 

/** 
 **************************************************************************************
 * @package com.springproject.dockerspring.SingleTests.FileIO.ZipProcess_Test.Audio_Data_ZipImpl_Test
 * 
 * @brief ZIPファイルの入出力単体テストのうち、[音源データ管理機能]に関するテストを行うクラスを格納した
 * パッケージ
 * 
 * @details
 * - このパッケージは、音源データ管理機能に関するCSVファイルの処理機能をテストするクラスを格納している。
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.FileIO.ZipProcess_Test.Audio_Data_ZipImpl_Test;
...
/** 
 **************************************************************************************
 * @file Audio_Data_ZipImpl_ExtractZip_Test.java
 * @brief 主に[音源データ管理]の圧縮ファイルの解凍抽出機能のテストを行うクラスを格納した
 * ファイル。
 * 
...
import com.springproject.dockerspring.FileIO.ZipProcess.Audio_Data_ZipImpl;
...
/** 
 **************************************************************************************
 * @brief 主に[音源データ管理]の圧縮ファイルの解凍抽出機能のテストを行うクラス
 * 
 * @details 
 * - このテストで対象となるクラスは、[Audio_Data_ZipImpl]である。
 * - 主に、集約クラス側の圧縮ファイルの解凍抽出機能をテストする。
 * 
 * @see Sound_Double_Component_TestUtils
 * @see ZipProcess_ExtractZip_Test
 * @see Audio_Data_ZipImpl
 * @see Audio_Data_Zip
 * @see Common_Zip
 * 
 ...
 **************************************************************************************
 */ 
@SpringBootTest(classes = {Audio_Data_TestCase_Make.class, Common_Zip.class})
public class Audio_Data_ZipImpl_ExtractZip_Test implements ZipProcess_ExtractZip_Test{

  private final Audio_Data_TestCase_Make testcase;
  private final Audio_Data_Zip audio_zip;

  private final SoftAssertions softly = new SoftAssertions();

  private final String target_func = "AudioData";
  private final String prefix = "AudioDataZipImplExtractZipTest";
   ...
   * - モックの環境変数において、[画像データ量制限値]は[209715200]として設定したうえでテストを行う。
   ...
   */
  @Autowired
  public Audio_Data_ZipImpl_ExtractZip_Test(Audio_Data_TestCase_Make testcase, Common_Zip common_zip){

    Environment_Config config = mock(Environment_Config.class);
    when(config.getAudio_list_size()).thenReturn(5);
    when(config.getAudio_limit()).thenReturn(209715200);

    this.audio_zip = new Audio_Data_ZipImpl(config, common_zip);
  ...
  public void 圧縮ファイルからのデータ抽出機能の成功パターンの確認() throws IOException, InputFileDifferException{
    ...
    File zip_file = Sound_Double_Component_TestUtils.generateTmpFile(this.target_func, 
                                                                     "compzip", 
    ...
    File compare_file_1 = Sound_Double_Component_TestUtils.
                          generateTmpFile(this.target_func, 
                                          "form", 
                                          this.testcase.getNormalData().get(Audio_Data_TestKeys.Audio_Data), 
                                          this.prefix, 
                                          "wav", 
                                          true);

    File compare_file_2 = Sound_Double_Component_TestUtils.
                          generateTmpFile(this.target_func, 
                                          "form", 
                                          this.testcase.getOkData().get(Audio_Data_TestKeys.Audio_Data), 
                                          this.prefix, 
                                          "wav", 
                                          true);
    ...
    try(BufferedInputStream bis = new BufferedInputStream(new FileInputStream(zip_file))){
      ...
      tmp_file_list = this.audio_zip.extractZip(mock);
    }
    ...
    for(FileEntity item: tmp_file_list){
      try(BufferedInputStream bis_zip = new BufferedInputStream(new FileInputStream(item.getTmpfile()));
        ...
        Sound_Double_Component_TestUtils.deleteTmpFileCheck(item.getTmpfile(), this.softly);
      }
    }
  ...
  public void 圧縮ファイルからのデータ抽出機能の失敗パターンの確認() throws IOException, InputFileDifferException{
    ...
    for(String file_name: fail_data){
      ...
      try(BufferedInputStream bis = new BufferedInputStream(new FileInputStream(zip_file))){
        ...
        this.softly.assertThatThrownBy(() -> this.audio_zip.extractZip(mock))
