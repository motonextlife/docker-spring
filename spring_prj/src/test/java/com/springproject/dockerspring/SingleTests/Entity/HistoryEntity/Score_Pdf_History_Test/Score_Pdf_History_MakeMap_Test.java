/** 
 **************************************************************************************
 * @package com.springproject.dockerspring.SingleTests.Entity.HistoryEntity.Score_Pdf_History_Test
 * 
 * @brief 履歴用エンティティのうち、[楽譜データ情報変更履歴]に関するテストを格納するパッケージ
 * 
 * @details
 * - このパッケージは、楽譜データ情報変更履歴のテストを行うクラスを格納する。
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.Entity.HistoryEntity.Score_Pdf_History_Test;
...
/** 
 **************************************************************************************
 * @file Score_Pdf_History_MakeMap_Test.java
 * 
 * @brief [楽譜データ変更履歴]において、エンティティ内部のデータを画面出力用に文字列に変換して
 ...
 * 
 * @details 
 * - 検査対象のクラス名は[Score_Pdf_History]で、検査対象メソッドは[makeMap]である。
...
public class Score_Pdf_History_MakeMap_Test implements Entity_MakeMap_Test{
  ...
  public void 入力した文字列マップリストデータの同一性確認() throws ParseException{
    ...
    Map<Score_Pdf_TestKeys, String> normal_case = testcase.getNormalData();
    ...
    String serial_num = normal_case.get(Score_Pdf_TestKeys.Serial_Num);
    String score_id = normal_case.get(Score_Pdf_TestKeys.Score_Id);
    String score_name = normal_case.get(Score_Pdf_TestKeys.Score_Name);
    ...
    entity.setScore_name(score_name);

    Map<String, String> comparemap = new HashMap<>();
    comparemap.put("history_id", history_id);
    comparemap.put("change_datetime", change_datetime);
    comparemap.put("change_kinds", change_kinds);
    comparemap.put("operation_user", operation_user);
    comparemap.put("serial_num", serial_num);
    comparemap.put("score_id", score_id);
    comparemap.put("score_name", score_name);

    this.softly.assertThat(comparemap).isEqualTo(entity.makeMap());
