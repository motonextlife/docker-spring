 ...
 * 
 * @brief 履歴用リポジトリのうち、[音源情報変更履歴]に関するテストを格納するパッケージ
 * 
 * @details
 * - このパッケージは、音源情報変更履歴のテストを行うクラスを格納する。
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.Repository.HistoryRepo.Sound_Source_History_Repo_Test;
...
/** 
 **************************************************************************************
 * @file Sound_Source_History_Repo_Delete_Test.java
 * @brief 主に[音源情報変更履歴]のリポジトリクラスで、データの削除を行うメソッドのテストを行う
 * クラスを格納したファイル。
 * 
 * @note
...
/** 
 **************************************************************************************
 * @brief 主に[音源情報変更履歴]のリポジトリクラスで、データの削除を行うメソッドのテストを
 * 行うクラス
 * 
 * @details 
...
public class Sound_Source_History_Repo_Delete_Test{
  ...
  public void testcaseSetup(Sound_Source_TestCase_Make testcase, 
                            Sound_Source_History_Repo test_repo){

    Sound_Source_History_Repo_Delete_Test.testcase = testcase;
    Sound_Source_History_Repo_Delete_Test.test_repo = test_repo;
    ...
    Sound_Source_History_Repo_Delete_Test.compare_entity = compare_entity;
  }
  ...
  public class Delete_Success {
    ...
    public Delete_Success() throws IOException{
      this.test_repo = Sound_Source_History_Repo_Delete_Test.test_repo;
      this.compare_entity = Sound_Source_History_Repo_Delete_Test.compare_entity;
      this.softly = Sound_Source_History_Repo_Delete_Test.softly;

      Sound_Source_History_Repo_Delete_Test.testcase.historyDatabaseSetup();
    }
    ...
    public void 削除の成功ケース(){
      ...
      for(int i = 0; i < 10; i++){
        Sound_Source_History del_ent = this.compare_entity.get(i);
      ...
      for(int i = 10; i < 20; i++){
        Sound_Source_History no_delete = this.compare_entity.get(i);

        Sound_Source_History result = this.test_repo.findById(no_delete.getSerial_num()).get();
    ...
    public static void testcaseReset() throws IOException{
      Sound_Source_History_Repo_Delete_Test.testcase.historyDatabaseReset();
    }
  }
  ...
  public class Period_Delete_Success {
    ...
    public void 該当期間データの削除の成功ケース() throws ParseException, InterruptedException{
      ...
      String[] assume = Sound_Source_History_Repo_Delete_Test.testcase
      ...
      List<Sound_Source_History> compare = Common_Repository_TestUtils
        .compareListMake(CompareParam
                         ...
                         .sort_first(Comparator.comparing(Sound_Source_History::getHistory_id))
                         .skip(0)
      ...
      List<Sound_Source_History> get_data = (List<Sound_Source_History>) this.test_repo.findAll();
      List<Sound_Source_History> result = get_data.stream()
                                                  .sorted(comparing(Sound_Source_History::getHistory_id))
      ...
      for(int i = 0; i < compare_size; i++){
        Sound_Quad_Repository_TestUtils.assertAllEquals(compare.get(i), result.get(i), this.softly, true);
      }
