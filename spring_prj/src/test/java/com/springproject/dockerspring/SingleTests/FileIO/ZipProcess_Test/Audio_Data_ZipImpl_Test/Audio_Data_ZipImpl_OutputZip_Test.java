...
public class Audio_Data_ZipImpl_OutputZip_Test implements ZipProcess_OutputZip_Test{
   ...
   */
  @Autowired
  public Audio_Data_ZipImpl_OutputZip_Test(Audio_Data_TestCase_Make testcase, Common_Zip common_zip){
  ...
  private List<FileEntity> makeCommpressFiles() throws IOException{
    ...
    String ok_data = testcase.getNormalData().get(Audio_Data_TestKeys.Audio_Data);
    List<FileEntity> tmp_file_list = new ArrayList<>();

    for(int i = 0; i <= 10; i++){
      File before_file = Sound_Double_Component_TestUtils.
  ...
  private void checkCompressData(File compress_file) throws IOException{
    ...
    File compare_file = Sound_Double_Component_TestUtils.
                        generateTmpFile(this.target_func, 
                                        this.target_dir, 
                                        testcase.getNormalData().get(Audio_Data_TestKeys.Audio_Data), 
                                        this.prefix, 
                                        this.extension, 
                                        true);
