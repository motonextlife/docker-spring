...
public class Facility_History_NormalEntityInput_Test implements Entity_NormalEntityInput_Test{
  ...
  private void setInitTargetEntity(String str){
    ...
    when(mock_entity.getFaci_id()).thenReturn(str);
    when(mock_entity.getFaci_name()).thenReturn(str);
    when(mock_entity.getProducer()).thenReturn(str);
    when(mock_entity.getStorage_loc()).thenReturn(str);
    when(mock_entity.getOther_comment()).thenReturn(str);
    
    assertAllNull(new Facility_History(mock_entity, null, str, null));
  }
  ...
  public void 入力した通常エンティティデータの同一性確認() throws ParseException{
    ...
    Facility mock_entity = mock(Facility.class);
    when(mock_entity.getSerial_num()).thenReturn(serial_num);
    when(mock_entity.getFaci_id()).thenReturn(faci_id);
    when(mock_entity.getFaci_name()).thenReturn(faci_name);
    when(mock_entity.getBuy_date()).thenReturn(buy_date);
    when(mock_entity.getProducer()).thenReturn(producer);
    when(mock_entity.getStorage_loc()).thenReturn(storage_loc);
    when(mock_entity.getDisp_date()).thenReturn(disp_date);
    when(mock_entity.getOther_comment()).thenReturn(other_comment);

    Facility_History entity = new Facility_History(mock_entity, 
  ...
  public void 登録履歴種別の同一性確認(){
    ...
    for(int i = 0; i < size; i++){
      entity = new Facility_History(mock_entity, hist_enum[i], null, null);
