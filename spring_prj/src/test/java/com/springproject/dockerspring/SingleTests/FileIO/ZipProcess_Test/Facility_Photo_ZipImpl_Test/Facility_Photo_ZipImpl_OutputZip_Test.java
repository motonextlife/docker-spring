 ...
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.FileIO.ZipProcess_Test.Facility_Photo_ZipImpl_Test;
...
public class Facility_Photo_ZipImpl_OutputZip_Test implements ZipProcess_OutputZip_Test{
   ...
   */
  @Autowired
  public Facility_Photo_ZipImpl_OutputZip_Test(Facility_Photo_TestCase_Make testcase, Common_Zip common_zip){
  ...
  private List<FileEntity> makeCommpressFiles() throws IOException{
    ...
    String ok_data = testcase.getNormalData().get(Facility_Photo_TestKeys.Photo_Data);
    List<FileEntity> tmp_file_list = new ArrayList<>();

    for(int i = 0; i <= 10; i++){
      File before_file = Facility_Double_Component_TestUtils.
  ...
  private void checkCompressData(File compress_file) throws IOException{
    ...
    File compare_file = Facility_Double_Component_TestUtils.
                        generateTmpFile(this.target_func, 
                                        this.target_dir, 
                                        testcase.getNormalData().get(Facility_Photo_TestKeys.Photo_Data), 
                                        this.prefix, 
                                        this.extension, 
                                        true);
  ...
  public void 圧縮ファイル出力機能の動作確認() throws IOException, InputFileDifferException{
    ...
    checkCompressData(this.faci_zip.outputZip(generate_func));
