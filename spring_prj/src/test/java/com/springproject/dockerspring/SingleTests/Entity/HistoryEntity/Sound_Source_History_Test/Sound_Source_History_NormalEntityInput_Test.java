 ...
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.Entity.HistoryEntity.Sound_Source_History_Test;
...
public class Sound_Source_History_NormalEntityInput_Test implements Entity_NormalEntityInput_Test{
  ...
  public void 入力した通常エンティティデータの同一性確認() throws ParseException{
    ...
    Sound_Source mockentity = mock(Sound_Source.class);
    when(mockentity.getSerial_num()).thenReturn(serial_num);
    when(mockentity.getSound_id()).thenReturn(sound_id);
    when(mockentity.getUpload_date()).thenReturn(upload_date);
    when(mockentity.getSong_title()).thenReturn(song_title);
    when(mockentity.getComposer()).thenReturn(composer);
    when(mockentity.getPerformer()).thenReturn(performer);
    when(mockentity.getPublisher()).thenReturn(publisher);
    when(mockentity.getOther_comment()).thenReturn(other_comment);

    Sound_Source_History entity = new Sound_Source_History(mockentity, 
  ...
  public void 登録履歴種別の同一性確認(){
    ...
    for(int i = 0; i < size; i++){
      entity = new Sound_Source_History(mock_entity, hist_enum[i], null, null);
