...
public class Facility_HistInput_Test implements Entity_HistInput_Test{
  ...
  private void setInitTargetEntity(String str){
    ...
    Facility_History mock_hist = mock(Facility_History.class);
    when(mock_hist.getSerial_num()).thenReturn(null);
    when(mock_hist.getFaci_id()).thenReturn(str);
    when(mock_hist.getFaci_name()).thenReturn(str);
    when(mock_hist.getProducer()).thenReturn(str);
    when(mock_hist.getStorage_loc()).thenReturn(str);
    when(mock_hist.getOther_comment()).thenReturn(str);

    assertAllNull(new Facility(mock_hist));
  }
  ...
  public void 入力した履歴用エンティティデータの同一性確認() throws ParseException{
    ...
    when(mock_hist.getFaci_id()).thenReturn(faci_id);
    when(mock_hist.getFaci_name()).thenReturn(faci_name);
    when(mock_hist.getBuy_date()).thenReturn(buy_date);
    when(mock_hist.getProducer()).thenReturn(producer);
    ...
    Facility entity = new Facility(mock_hist);
