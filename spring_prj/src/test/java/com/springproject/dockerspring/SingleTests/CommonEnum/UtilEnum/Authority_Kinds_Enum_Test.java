/** 
 **************************************************************************************
 * @package com.springproject.dockerspring.SingleTests.CommonEnum.UtilEnum
 * 
 * @brief 列挙型の単体テストのうち、[ユーティリティ列挙型]に関するテストを格納する。
 * 
 * @details
 * - このパッケージは、システム全体で用いるファイルパス文字列や正規表現文字列などのユーティリティを
 * 定義した列挙型のテストを格納する。
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.CommonEnum.UtilEnum;





/** 
 **************************************************************************************
 * @file Authority_Kinds_Enum_Test.java
 * 
 * @brief 主に[権限情報の種別定義]機能において使用する列挙型をテストするクラスを格納したファイル
 * である。
 * 
 * @note
 * このファイルに、所属パッケージの説明を記載する。
 **************************************************************************************
 */ 
import org.junit.jupiter.api.Test;

import com.springproject.dockerspring.CommonEnum.UtilEnum.Authority_Kinds_Enum;

import org.assertj.core.api.SoftAssertions;








/** 
 **************************************************************************************
 * @brief 主に[権限情報の種別定義]機能において使用する列挙型をテストするクラス。
 * 
 * @details 
 * - 検査対象の列挙型名は[Authority_Kinds_Enum]である。
 * - テスト内容としては、主に想定された値が列挙型に格納されていることを確認する。
 * 
 * @par 大まかなテストの手順
 * - 列挙型に格納されている値の比較用として、想定される値を想定される順番通りに格納した配列を用意。
 * - 比較用配列と検査対象列挙型から順番に値を取り出していき、値が完全一致することを確認。
 * 
 * @see Authority_Kinds_Enum
 **************************************************************************************
 */ 
public class Authority_Kinds_Enum_Test {

  private final String[] kinds_order = {
    "true",
    "false",
    "none",
    "brows",
    "change",
    "delete",
    "hist",
    "rollback",
    "error"
  };

  private final SoftAssertions softly = new SoftAssertions();

  



  /** 
   ****************************************************************************************
   * @brief 各列挙型項目の一致テストを行う。
   ****************************************************************************************
   */ 
  @Test
  public void 各列挙型項目の一致テスト(){
    Authority_Kinds_Enum[] enumlist = Authority_Kinds_Enum.values();
    int size = this.kinds_order.length;

    for(int i = 0; i < size; i++){
      this.softly.assertThat(enumlist[i].getKinds())
                 .isEqualTo(this.kinds_order[i]);
    }
        
    this.softly.assertAll();
  }
}