/** 
 **************************************************************************************
 * @file Score_Pdf_SambaImpl_FileOutput_Test.java
 * @brief 主に[楽譜データ管理]のファイルサーバーのデータ出力機能のテストを行うクラスを格納した
 * ファイル。
 **************************************************************************************
 */ 
...
/** 
 **************************************************************************************
 * @brief 主に[楽譜データ管理]のファイルサーバーのデータ出力機能のテストを行うクラス
 * 
 * @details 
 ...
 * @see Score_Pdf_SambaImpl_FileOutput_Test
...
public class Score_Pdf_SambaImpl_FileOutput_Test implements SambaProcess_FileOutput_Test{
  ...
  private final String prefix = "ScorePdfSambaImplFileOutputTest";
   ...
   * @param[in] score_samba テスト対象の集約クラス
   ...
   */
  @Autowired
  public Score_Pdf_SambaImpl_FileOutput_Test(Score_Pdf_TestCase_Make testcase, 
    ...
    String other_data = this.testcase.getPdfDataFailedData().get(0);

    this.normal_file = Score_Double_Component_TestUtils.
    ...
    this.other_file = Score_Double_Component_TestUtils.
    ...
    String normal_path_1 = Score_Double_Component_TestUtils.sambaMakePath("1", this.ext, false, this.config);
    String normal_path_2 = Score_Double_Component_TestUtils.sambaMakePath("2", this.ext, false, this.config);
    String normal_dir = Score_Double_Component_TestUtils.sambaMakePath("3", this.ext, false, this.config);
    String history_path_1 = Score_Double_Component_TestUtils.sambaMakePath("1", this.hist_ext, true, this.config);
    String history_path_2 = Score_Double_Component_TestUtils.sambaMakePath("2", this.hist_ext, true, this.config);
    String history_dir = Score_Double_Component_TestUtils.sambaMakePath("3", this.hist_ext, true, this.config);
    
    Score_Double_Component_TestUtils.preparaSambaData(this.normal_file, normal_path_1, this.config, false);
    Score_Double_Component_TestUtils.preparaSambaData(this.other_file, normal_path_2, this.config, false);
    Score_Double_Component_TestUtils.preparaSambaData(null, normal_dir, this.config, true);
    Score_Double_Component_TestUtils.preparaSambaData(this.hist_file, history_path_1, this.config, false);
    Score_Double_Component_TestUtils.preparaSambaData(this.hist_other_file, history_path_2, this.config, false);
    Score_Double_Component_TestUtils.preparaSambaData(null, history_dir, this.config, true);
  }
  ...
  private void configStaticSet(){
    Score_Pdf_SambaImpl_FileOutput_Test.config_static = this.config;
  }
  ...
  public void 集約クラス側のファイルサーバーからの通常データ出力機能の動作確認() throws IOException, InputFileDifferException{
    ...
    File normal_1 = this.score_samba.fileOutput(1, this.score_samba.makeHash(this.normal_file));
    fileCheck(this.normal_file, normal_1);

    File normal_2 = this.score_samba.fileOutput(2, this.score_samba.makeHash(this.other_file));
    fileCheck(this.other_file, normal_2);

    this.softly.assertThatThrownBy(() -> this.score_samba.fileOutput(1, "blank"))
               .isInstanceOf(InputFileDifferException.class);
    this.softly.assertThatThrownBy(() -> this.score_samba.fileOutput(3, "blank"))
               .isInstanceOf(IOException.class);
    this.softly.assertThatThrownBy(() -> this.score_samba.fileOutput(4, "blank"))
               .isInstanceOf(IOException.class);
  ...
  public void 集約クラス側のファイルサーバーからの履歴データ出力機能の動作確認() throws IOException, InputFileDifferException{
    ...
    File history_1 = this.score_samba.fileOutput(1, this.score_samba.makeHash(this.normal_file));
    fileCheck(this.normal_file, history_1);

    File history_2 = this.score_samba.fileOutput(2, this.score_samba.makeHash(this.other_file));
    fileCheck(this.other_file, history_2);

    this.softly.assertThatThrownBy(() -> this.score_samba.historyFileOutput(1, "blank"))
               .isInstanceOf(InputFileDifferException.class);
    this.softly.assertThatThrownBy(() -> this.score_samba.historyFileOutput(3, "blank"))
               .isInstanceOf(IOException.class);
    this.softly.assertThatThrownBy(() -> this.score_samba.historyFileOutput(4, "blank"))
               .isInstanceOf(IOException.class);
  ...
  public static void ファイルサーバーと一時ファイルディレクトリの初期化確認() throws InterruptedException, IOException{
    Score_Double_Component_TestUtils.resetDir(Score_Pdf_SambaImpl_FileOutput_Test.config_static);
    Score_Double_Component_TestUtils.checkTmpDirClear();
    Score_Double_Component_TestUtils.checkSambaClear(Score_Pdf_SambaImpl_FileOutput_Test.config_static);
  }
}
