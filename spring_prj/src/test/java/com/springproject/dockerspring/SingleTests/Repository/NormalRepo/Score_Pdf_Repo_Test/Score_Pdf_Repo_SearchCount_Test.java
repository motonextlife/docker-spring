/** 
 **************************************************************************************
 * @file Score_Pdf_Repo_SearchCount_Test.java
 * @brief 主に[楽譜データ管理]のリポジトリクラスで、独自に追加した検索結果を取得するメソッドと
...
public class Score_Pdf_Repo_SearchCount_Test{
  ...
  private final List<Score_Pdf> origin_compare;
  ...
  //! 検索ワード
  public final int WORD = Score_Pdf_RepoForm_TestCase.WORD;

  //! 1ページ当たりの検索結果の出力限界数
  public final int LIMIT = Score_Pdf_RepoForm_TestCase.LIMIT;

  //! 検索結果の全件数を出力した際の結果数想定値
  public final int ANSWER_COUNT = Score_Pdf_RepoForm_TestCase.ANSWER_COUNT;

  //! オフセット値
  public final int OFFSET = Score_Pdf_RepoForm_TestCase.OFFSET;
   ...
   */
  @Autowired
  public Score_Pdf_Repo_SearchCount_Test(Score_Pdf_Repo test_repo, 
                                         Score_Pdf_TestCase_Make testcase, 
                                         Musical_Score_TestCase_Make foreign_case) throws IOException{
  ...
  private void configStaticSet(){
    Score_Pdf_Repo_SearchCount_Test.testcase_static = this.testcase;
    Score_Pdf_Repo_SearchCount_Test.foreign_case_static = this.foreign_case;
  }
   ...
   **********************************************************************************************
   */
  private void findAllSearchCheck(Function<TestParam<Score_Pdf>, CompletableFuture<List<Score_Pdf>>> find_all_method, 
                                  TestParamBuilder<Score_Pdf> test_builder) 
                                  throws ExecutionException, InterruptedException{

    BiConsumer<Score_Pdf, Score_Pdf> assert_method = (compare, result) -> {
      Score_Quad_Repository_TestUtils.assertAllEquals(compare, result, this.softly, true);
    };

    TestParam<Score_Pdf> test_param = test_builder.assert_method(assert_method)
                                                  .softly(this.softly)
                                                  .build();
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findByScore_id][findByScore_idDESC]を実行した際の検索結果を判定する。
   *
   * @details
   ...
   */
  @Test
  public void 指定された楽譜番号の保存されているデータを全取得する際のテスト() throws InterruptedException, ExecutionException{

    for(int i = 1; i <= 20; i++){
      String[] assume = this.testcase.otherRepoForm(Score_Pdf_TestKeys.Score_Id, i);

      String word = assume[WORD];
      int limit = Integer.parseInt(assume[LIMIT]);
      int offset = Integer.parseInt(assume[OFFSET]);

      CompareParam<Score_Pdf> list_builder = CompareParam
                              .<Score_Pdf>builder()
                              .origin_compare(this.origin_compare)
                              .filter(s -> s.getScore_id().equals(word))
                              .skip(offset)
                              .limit(limit)
                              .build();

      List<Score_Pdf> compare_1 = Common_Repository_TestUtils
                                  .compareListMake(list_builder.toBuilder()
                                                               .sort_first(Comparator.comparing(Score_Pdf::getScore_name))
                                                               .sort_second(Comparator.comparing(Score_Pdf::getSerial_num))
                                                               .build());
      List<Score_Pdf> compare_2 = Common_Repository_TestUtils
                                  .compareListMake(list_builder.toBuilder()
                                                               .sort_first(Comparator.comparing(Score_Pdf::getScore_name, 
      ...
      Function<TestParam<Score_Pdf>, CompletableFuture<List<Score_Pdf>>> find_all_method_1 = s -> {
        return this.test_repo.findByScore_id(s.getMain_word(), s.getLimit(), s.getOffset());
      };

      Function<TestParam<Score_Pdf>, CompletableFuture<List<Score_Pdf>>> find_all_method_2 = s -> {
        return this.test_repo.findByScore_idDESC(s.getMain_word(), s.getLimit(), s.getOffset());
      };

      TestParam<Score_Pdf> test_builder = TestParam.<Score_Pdf>builder().main_word(word)
                                                                        .limit(limit)
                                                                        .offset(offset)
                                                                        .all_judge(false)
                                                                        .build();
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findAllByScore_name][findAllByScore_nameDESC]を実行した際の
   * 検索結果を判定する。
   *
   * @details
   * - テストの内容としては、検索ワードを渡してメソッドを実行した際に、想定された検索結果と、テスト対象メソッド
   ...
   */
  @Test
  public void findAllByScore_nameTest() throws InterruptedException, ExecutionException{

    for(int i = 1; i <= 20; i++){
      String[] assume_id = this.testcase.otherRepoForm(Score_Pdf_TestKeys.Score_Id, i);
      String[] assume_name = this.testcase.otherRepoForm(Score_Pdf_TestKeys.Score_Name, i);
      ...
      CompareParam<Score_Pdf> list_builder = CompareParam
                              .<Score_Pdf>builder()
                              .origin_compare(this.origin_compare)
                              .filter(s -> s.getScore_id().equals(id)
                                        && s.getScore_name().contains(word))
                              .skip(offset)
                              .limit(limit)
                              .build();
      ...
      Function<TestParam<Score_Pdf>, CompletableFuture<List<Score_Pdf>>> find_all_method_1 = s -> {
        return this.test_repo.findAllByScore_name(s.getMain_word(), s.getSub_word(), s.getLimit(), s.getOffset());
      };

      Function<TestParam<Score_Pdf>, CompletableFuture<List<Score_Pdf>>> find_all_method_2 = s -> {
        return this.test_repo.findAllByScore_nameDESC(s.getMain_word(), s.getSub_word(), s.getLimit(), s.getOffset());
      };

      TestParam<Score_Pdf> test_builder = TestParam.<Score_Pdf>builder().main_word(id)
                                                                        .sub_word(word)
                                                                        .limit(limit)
                                                                        .offset(offset)
                                                                        .all_judge(false)
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findByScore_idCount]に対してのテストを行う。
   *
   * @details
   ...
   */
  @Test
  public void カウント対象のカラムが楽譜番号の際のテスト() throws InterruptedException, ExecutionException{

    for(int i = 1; i <= 20; i++){
      String[] assume = this.testcase.otherRepoForm(Score_Pdf_TestKeys.Score_Id, i);

      String score_id = assume[WORD];
      int limit = Integer.parseInt(assume[LIMIT]);
      int answer = Integer.parseInt(assume[ANSWER_COUNT]);

      CompletableFuture<Integer> result_1 = this.test_repo.findByScore_idCount(score_id, limit);
      CompletableFuture<Integer> result_2 = this.test_repo.findByScore_idCount(null, limit);
      CompletableFuture<Integer> result_3 = this.test_repo.findByScore_idCount(score_id, null);
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findAllByScore_nameCount]に対してのテストを行う。
   *
   * @details
   * - テストの内容としては、出力件数最大値や検索ワードを切り替えて検索した場合、検索結果件数が想定通りの
   * 値であることを確認する。
   ...
   */
  @Test
  public void カウント対象のカラムが楽譜ファイル名の際のテスト() throws InterruptedException, ExecutionException{

    for(int i = 1; i <= 20; i++){
      String[] assume_id = this.testcase.otherRepoForm(Score_Pdf_TestKeys.Score_Id, i);
      String[] assume_name = this.testcase.otherRepoForm(Score_Pdf_TestKeys.Score_Name, i);

      String score_id = assume_id[WORD];
      String word = assume_name[WORD];
      int limit = Integer.parseInt(assume_name[LIMIT]);
      int answer = Integer.parseInt(assume_name[ANSWER_COUNT]);

      CompletableFuture<Integer> result_1 = this.test_repo.findAllByScore_nameCount(score_id, word, limit);
      CompletableFuture<Integer> result_2 = this.test_repo.findAllByScore_nameCount(score_id, null, limit);
      CompletableFuture<Integer> result_3 = this.test_repo.findAllByScore_nameCount(null, word, limit);
      CompletableFuture<Integer> result_4 = this.test_repo.findAllByScore_nameCount(score_id, word, null);
  ...
  public static void testcaseReset() throws IOException{
    Score_Pdf_Repo_SearchCount_Test.testcase_static.databaseReset(false);
    Score_Pdf_Repo_SearchCount_Test.foreign_case_static.databaseReset();
  }
}
