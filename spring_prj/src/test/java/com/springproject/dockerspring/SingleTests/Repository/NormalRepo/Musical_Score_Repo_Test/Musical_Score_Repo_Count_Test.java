 ...
 * 
 * @brief 通常用リポジトリのうち、[楽譜情報]に関するテストを格納するパッケージ
 * 
 * @details
 * - このパッケージは、楽譜情報のテストを行うクラスを格納する。
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.Repository.NormalRepo.Musical_Score_Repo_Test;
...
/** 
 **************************************************************************************
 * @file Musical_Score_Repo_Count_Test.java
 * @brief 主に[楽譜管理]のリポジトリクラスで、検索結果のカウントを行うメソッドのテストを行う
 * クラスを格納したファイル。
 * 
 * @note
...
/** 
 **************************************************************************************
 * @brief 主に[楽譜管理]のリポジトリクラスで、検索結果のカウントを行うメソッドのテストを行うクラス
 * 
 * @details 
...
public class Musical_Score_Repo_Count_Test{
   ...
   */
  @Autowired
  public Musical_Score_Repo_Count_Test(Musical_Score_Repo test_repo, Musical_Score_TestCase_Make testcase) throws IOException{
  ...
  private void configStaticSet(){
    Musical_Score_Repo_Count_Test.testcase_static = this.testcase;
  }
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findAllByScore_idCount]に対してのテストを行う。
   *
   * @details
  ...
  public void カウント対象のカラムが楽譜番号の際のテスト() throws InterruptedException, ExecutionException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      CompletableFuture<Integer> result_1 = this.test_repo.findAllByScore_idCount(word, limit);
      CompletableFuture<Integer> result_2 = this.test_repo.findAllByScore_idCount(null, limit);
      CompletableFuture<Integer> result_3 = this.test_repo.findAllByScore_idCount(word, null);
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findAllByArrangerCount]に対してのテストを行う。
   *
   * @details
   * - テストの内容としては、出力件数最大値や検索ワードを切り替えて検索した場合、検索結果件数が想定通りの
   * 値であることを確認する。
   ...
   */
  @Test
  public void カウント対象のカラムが編曲者の際のテスト() throws InterruptedException, ExecutionException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      CompletableFuture<Integer> result_1 = this.test_repo.findAllByArrangerCount(word, limit);
      CompletableFuture<Integer> result_2 = this.test_repo.findAllByArrangerCount(null, limit);
      CompletableFuture<Integer> result_3 = this.test_repo.findAllByArrangerCount(word, null);
  ...
  public static void testcaseReset() throws IOException{
    Musical_Score_Repo_Count_Test.testcase_static.databaseReset();
  }
}
