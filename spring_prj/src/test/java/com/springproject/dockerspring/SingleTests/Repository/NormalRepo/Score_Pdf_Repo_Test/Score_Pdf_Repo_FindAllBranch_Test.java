/** 
 **************************************************************************************
 * @file Score_Pdf_Repo_FindAllBranch_Test.java
...
public class Score_Pdf_Repo_FindAllBranch_Test{
   ...
   */
  @Autowired
  public Score_Pdf_Repo_FindAllBranch_Test(Score_Pdf_Repo test_repo, 
  ...
  private void configStaticSet(){
    Score_Pdf_Repo_FindAllBranch_Test.testcase_static = this.testcase;
    Score_Pdf_Repo_FindAllBranch_Test.foreign_case_static = this.foreign_case;
  }
   ...
   **********************************************************************************************
   */
  private void findAllBranchCheck(TestParamBuilder<Score_Pdf> test_builder) 
  ...
  public void 検索分岐メソッドにおいて検索種別が全検索の際のテスト() throws ExecutionException, InterruptedException, ParseException{
    ...
    String subject = this.testcase.subjectRepoForm(Score_Pdf_RepoForm_TestCase.ALL);
    ...
    for(int i = 1; i <= 20; i++){
      ...
      TestParam<Score_Pdf> test_builder = TestParam.<Score_Pdf>builder()
                                                   .main_word(word)
                                                   .subject(subject)
                                                   .max(limit)
                                                   .limit(limit)
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findAllBranch]において、検索種別を[ScoreName]にした時の検索結果を判定する。
   *
   * @details
   * - テストの内容としては、検索ワードや検索種別を指定して検索をした場合、想定された検索結果と、
   * テスト対象メソッドから返ってきた値が、順番含めて一致することを確認する。
   ...
   */
  @Test
  public void 検索分岐メソッドにおいて検索種別がPDFファイル名の際のテスト() throws ExecutionException, InterruptedException, ParseException{

    String subject = this.testcase.subjectRepoForm(Score_Pdf_RepoForm_TestCase.SCORE_NAME);
  ...
  public void 検索分岐メソッドにおいて検索種別が不適合の際のエラーテスト(){
    ...
    String subject = this.testcase.subjectRepoForm(Score_Pdf_RepoForm_TestCase.MISSING);
  ...
  public static void testcaseReset() throws IOException{
    Score_Pdf_Repo_FindAllBranch_Test.testcase_static.databaseReset(false);
    Score_Pdf_Repo_FindAllBranch_Test.foreign_case_static.databaseReset();
  }
}
