/** 
 **************************************************************************************
 * @file Musical_Score_CsvImpl_ExtractCsv_Test.java
 * @brief 主に[楽譜番号]のCSVデータのデータ抽出機能のテストを行うクラスを格納した
 * ファイル。
 * 
...
/** 
 **************************************************************************************
 * @brief 主に[楽譜番号]のCSVデータのデータ抽出機能のテストを行うクラス
 * 
 * @details 
...
public class Musical_Score_CsvImpl_ExtractCsv_Test implements CsvProcess_ExtractCsv_Test{
  ...
  private final String prefix = "MusicalScoreCsvImplExtractCsvTest";
   ...
   */
  @Autowired
  public Musical_Score_CsvImpl_ExtractCsv_Test(Musical_Score_TestCase_Make testcase){
  ...
  public void 共通クラス側のCSV入力データ抽出機能確認の抽出成功パターン() throws IOException, InputFileDifferException{
    ...
    File ok_file = Score_Double_Component_TestUtils.generateTmpFile(this.target_func, 
                                                                    this.target_dir, 
  ...
  public void 集約クラス側のCSV入力データ抽出機能確認の抽出成功パターン() throws IOException, InputFileDifferException{
    ...
    try(FileInputStream fis = new FileInputStream(ok_file);
      ...
      MockMultipartFile mock_file = new MockMultipartFile("no_name", bis);
      tmp_file_path = this.score_csv.extractCsv(mock_file);
    }
  ...
  public void 共通クラス側のCSV入力データ抽出機能確認の抽出失敗パターン() throws IOException, InputFileDifferException{
    ...
    for(String filename: fail_list){
      File fail_file = Score_Double_Component_TestUtils.generateTmpFile(this.target_func, 
                                                                        this.target_dir, 
                                                                        filename, 
                                                                        this.prefix, 
                                                                        this.extension, 
  ...
  public void 集約クラス側のCSV入力データ抽出機能確認の抽出失敗パターン() throws IOException, InputFileDifferException{
    ...
    for(String filename: fail_list){
      ...
      try(BufferedInputStream bis = new BufferedInputStream(new FileInputStream(fail_file))){
        ...
        MockMultipartFile mock_file = new MockMultipartFile("no_name", bis);
        this.softly.assertThatThrownBy(() -> this.score_csv.extractCsv(mock_file))
                   .isInstanceOf(InputFileDifferException.class);
      }
    }

    this.softly.assertThatThrownBy(() -> this.score_csv.extractCsv(null))
               .isInstanceOf(NullPointerException.class);
