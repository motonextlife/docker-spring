/** 
 **************************************************************************************
 * @file Member_Info_History_Repo_SearchCount_Test.java
 * @brief 主に[団員情報変更履歴]のリポジトリクラスで、独自に追加した検索結果を取得するメソッドと
...
public class Member_Info_History_Repo_SearchCount_Test{
  ...
  private final List<Member_Info_History> origin_compare;
   ...
   */
  @Autowired
  public Member_Info_History_Repo_SearchCount_Test(Member_Info_History_Repo test_repo, 
  ...
  private void configStaticSet(){
    Member_Info_History_Repo_SearchCount_Test.testcase_static = this.testcase;
  }
   ...
   **********************************************************************************************
   */
  private void findAllSearchCheck(Function<TestParam<Member_Info_History>, CompletableFuture<List<Member_Info_History>>> find_all_method, 
                                  TestParamBuilder<Member_Info_History> test_builder) 
                                  throws ExecutionException, InterruptedException{

    BiConsumer<Member_Info_History, Member_Info_History> assert_method = (compare, result) -> {
      Member_Quad_Repository_TestUtils.assertAllEquals(compare, result, this.softly, true);
    };

    TestParam<Member_Info_History> test_param = test_builder.assert_method(assert_method)
                                                           .softly(this.softly)
                                                           .build();
  ...
  public void 検索対象のカラムが履歴日時の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      CompareParam<Member_Info_History> list_builder = CompareParam
      ...
      List<Member_Info_History> compare_1 = Common_Repository_TestUtils
                                .compareListMake(list_builder.toBuilder()
                                                             .sort_first(Comparator.comparing(Member_Info_History::getChange_datetime))
                                                             .sort_second(Comparator.comparing(Member_Info_History::getHistory_id))
                                                             .build());
      List<Member_Info_History> compare_2 = Common_Repository_TestUtils
                                .compareListMake(list_builder.toBuilder()
                                                             .sort_first(Comparator.comparing(Member_Info_History::getChange_datetime, 
      ...
      Function<TestParam<Member_Info_History>, CompletableFuture<List<Member_Info_History>>> find_all_method_1 = s -> {
        return this.test_repo.findAllByDateBetween(s.getStart_datetime(), s.getEnd_datetime(), s.getLimit(), s.getOffset());
      };

      Function<TestParam<Member_Info_History>, CompletableFuture<List<Member_Info_History>>> find_all_method_2 = s -> {
        return this.test_repo.findAllByDateBetweenDESC(s.getStart_datetime(), s.getEnd_datetime(), s.getLimit(), s.getOffset());
      };

      TestParam<Member_Info_History> test_builder = TestParam.<Member_Info_History>builder().start_datetime(start)
                                                                                            .end_datetime(end)
                                                                                            .limit(limit)
                                                                                            .offset(offset)
                                                                                            .all_judge(true)
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findByMember_id][findByMember_idDESC]を実行した際の
   * 検索結果を判定する。
   *
   * @details
   * - テストの内容としては、検索ワードを渡してメソッドを実行した際に、想定された検索結果と、テスト対象メソッド
   ...
   */
  @Test
  public void 検索対象のカラムが団員番号の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      List<Member_Info_History> compare_1 = Common_Repository_TestUtils
                                .compareListMake(list_builder.toBuilder()
                                                             .sort_first(Comparator.comparing(Member_Info_History::getMember_id))
                                                             .sort_second(Comparator.comparing(Member_Info_History::getChange_datetime))
                                                             .sort_third(Comparator.comparing(Member_Info_History::getHistory_id))
                                                             .build());
      List<Member_Info_History> compare_2 = Common_Repository_TestUtils
                                .compareListMake(list_builder.toBuilder()
                                                             .sort_first(Comparator.comparing(Member_Info_History::getMember_id, 
      ...
      Function<TestParam<Member_Info_History>, CompletableFuture<List<Member_Info_History>>> find_all_method_1 = s -> {
        return this.test_repo.findByMember_id(s.getMain_word(), s.getStart_datetime(), s.getEnd_datetime(), 
                                              s.getLimit(), s.getOffset());
      };

      Function<TestParam<Member_Info_History>, CompletableFuture<List<Member_Info_History>>> find_all_method_2 = s -> {
        return this.test_repo.findByMember_idDESC(s.getMain_word(), s.getStart_datetime(), s.getEnd_datetime(), 
                                                  s.getLimit(), s.getOffset());
      };

      TestParam<Member_Info_History> test_builder = TestParam.<Member_Info_History>builder().main_word(word)
  ...
  public void 検索対象のカラムが操作ユーザー名の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      List<Member_Info_History> compare_1 = Common_Repository_TestUtils
                                .compareListMake(list_builder.toBuilder()
                                                             .sort_first(Comparator.comparing(Member_Info_History::getOperation_user))
                                                             .sort_second(Comparator.comparing(Member_Info_History::getChange_datetime))
                                                             .sort_third(Comparator.comparing(Member_Info_History::getHistory_id))
                                                             .build());
      List<Member_Info_History> compare_2 = Common_Repository_TestUtils
  ...
  /** 
   **********************************************************************************************
   * @brief リポジトリメソッド[findByMember_idCount]に対してのテストを行う。
   *
   * @details
   * - テストの内容としては、出力件数最大値や検索ワードを切り替えて検索した場合、検索結果件数が想定通りの
   * 値であることを確認する。
   ...
   */
  @Test
  public void カウント対象のカラムが団員番号の際のテスト() throws InterruptedException, ExecutionException, ParseException{
    ...
    for(int i = 1; i <= 20; i++){
      ...
      CompletableFuture<Integer> result_1 = this.test_repo.findByMember_idCount(word, start, end, limit);
      CompletableFuture<Integer> result_2 = this.test_repo.findByMember_idCount(word, start, null, limit);
      CompletableFuture<Integer> result_3 = this.test_repo.findByMember_idCount(word, null, end, limit);
      CompletableFuture<Integer> result_4 = this.test_repo.findByMember_idCount(null, start, end, limit);
      CompletableFuture<Integer> result_5 = this.test_repo.findByMember_idCount(word, start, end, null);
  ...
  public static void testcaseReset() throws IOException{
    Member_Info_History_Repo_SearchCount_Test.testcase_static.historyDatabaseReset();
  }
}
