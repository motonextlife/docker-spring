 ...
 * 
 * @brief ファイルサーバーへの入出力単体テストのうち、[楽譜データ管理機能]に関するテストを行うクラスを格納した
 * パッケージ
 * 
 * @details
 * - このパッケージは、楽譜データ管理機能に関するファイルサーバーへの処理機能をテストするクラスを格納している。
 **************************************************************************************
 */ 
package com.springproject.dockerspring.SingleTests.FileIO.SambaProcess_Test.Score_Pdf_SambaImpl_Test;
...
/** 
 **************************************************************************************
 * @file Score_Pdf_SambaImpl_Compress_Test.java
 * @brief 主に[楽譜データ管理]のバイナリデータ圧縮解凍機能のテストを行うクラスを格納した
 * ファイル。
 * 
 * @note
...
/** 
 **************************************************************************************
 * @brief 主に[楽譜データ管理]のバイナリデータ圧縮解凍機能のテストを行うクラス
 * 
 * @details 
 ...
 **************************************************************************************
 */ 
@SpringBootTest(classes = {Score_Pdf_TestCase_Make.class, Environment_Config.class, Common_Samba.class})
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class Score_Pdf_SambaImpl_Compress_Test implements SambaProcess_Compress_Test{
  ...
  private final String prefix = "ScorePdfSambaImplCompressTest";
  ...
  public void バイナリデータの圧縮と解凍機能の動作確認() throws IOException{
    ...
    File source = Score_Double_Component_TestUtils.
                  generateTmpFile(this.target_func, this.target_dir, normal_data, 
                                  this.prefix, this.extension, true);

    File temp_file = Score_Double_Component_TestUtils.
                     generateTmpFile(this.target_func, this.target_dir, normal_data, 
    ...
    try(BufferedInputStream bis_1 = new BufferedInputStream(new FileInputStream(source));
      ...
      Score_Double_Component_TestUtils.deleteTmpFileCheck(decompress, this.softly);
    }
