/** 
 **************************************************************************************
 * @file Audio_Data_ZipImpl_OutputZip_Test.java
 * @brief 主に[音源データ管理]の圧縮ファイルの出力機能のテストを行うクラスを格納した
 * ファイル。
 * 
...
/** 
 **************************************************************************************
 * @brief 主に[音源データ管理]の圧縮ファイルの出力機能のテストを行うクラス
 * 
 * @details 
...
@SpringBootTest(classes = {Audio_Data_TestCase_Make.class, 
                           Sound_Source_TestCase_Make.class, 
                           Common_Zip.class, 
                           Audio_Data_SambaImpl.class, 
                           Common_Samba.class, 
                           Environment_Config.class})
public class Audio_Data_ZipImpl_OutputZip_Test implements ZipProcess_OutputZip_Test{
  ...
  private final String prefix = "AudioDataZipImplOutputZipTest";
   ...
   */
  @Autowired
  public Audio_Data_ZipImpl_OutputZip_Test(Audio_Data_TestCase_Make testcase, 
  ...
  private void configStaticSet(){
    Audio_Data_ZipImpl_OutputZip_Test.testcase_static = this.testcase;
    Audio_Data_ZipImpl_OutputZip_Test.foreign_case_static = this.foreign_case;
    Audio_Data_ZipImpl_OutputZip_Test.samba_config_static = this.samba_config;
  }
  ...
  private List<FileEntity> makeCommpressFiles() throws IOException, InputFileDifferException{
    ...
    for(int i = 0; i < 20; i++){
      String compare_hash = this.testcase.compareEntityMake(i + 1, true).getAudio_hash();
      File before_file = this.audio_samba.fileOutput(i + 1, compare_hash);
  ...
  private void checkCompressData(File compress_file) throws IOException{
    ...
    List<FileEntity> thawing = Sound_Double_Component_TestUtils
    ...
    for(int i = 1; i <= 20; i++){
      File compare_file = Sound_Double_Component_TestUtils.
  ...
  public void 圧縮ファイル出力機能の動作確認() throws IOException, InputFileDifferException{
    ...
    checkCompressData(this.audio_zip.outputZip(generate_func));
  ...
  public static void 一時ファイル用のディレクトリとファイルサーバーの初期化確認() throws InterruptedException, IOException{
    Audio_Data_ZipImpl_OutputZip_Test.testcase_static.databaseReset(true);
    Audio_Data_ZipImpl_OutputZip_Test.foreign_case_static.databaseReset();
    Sound_Double_Component_TestUtils.checkTmpDirClear();
    Sound_Double_Component_TestUtils.checkSambaClear(Audio_Data_ZipImpl_OutputZip_Test.samba_config_static);
  }
}
